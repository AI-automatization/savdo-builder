...
## 🔴 Новые задачи для Полата (18.04.2026)

| ID | Приоритет | Что | Где | Детали |
|---|---|---|---|---|
| **TMA-EDIT-001** | 🔴 Блокер | Чёрный экран при тапе на товар в seller-панели TMA | `apps/tma/src/pages/seller/EditProductPage.tsx:18-24, 629-631` | Регрессия от API-VAR-001. `normalizeVariant` (`products.controller.ts:532`) удаляет `optionValues` и отдаёт только `optionValueIds: string[]`, но interface `Variant` в EditProductPage всё ещё ждёт старую форму `optionValues: Array<{ optionValue }>`. `v.optionValues.length` → `TypeError` → React unmount → чёрный экран. Триггерится только на товарах **с вариантами**. Фикс: (1) заменить поле на `optionValueIds: string[]`, (2) `label` собирать из `product.options[].values[]` по id (или добавить `optionValueLabels: string[]` в `normalizeVariant`), (3) проверить остальные места чтения `v.optionValues` в TMA. Скриншот: `c:/Users/marti/Desktop/Снимок экрана 2026-04-17 112143.png`. Подробный лог: `analiz/logs.md` → TMA-EDIT-001. |
| **TMA-ERR-BOUNDARY-001** | 🟡 Инфраструктура | Добавить Error Boundary в TMA | `apps/tma/src/App.tsx` (обернуть `<Routes>`) | Сейчас любой runtime-crash = пустой чёрный фон + native Telegram BackButton прячется в cleanup упавшего компонента → пользователь видит только X. Нужен React Error Boundary с fallback UI: «Что-то пошло не так» + кнопка «Назад к главной» (`navigate('/')` + `tg?.BackButton.show()`). Опционально Sentry-лог. Спасает не только TMA-EDIT-001, но и все будущие регрессии в TMA. |
| **API-SUMMARY-500-001** | 🔴 Блокер | `GET /analytics/seller/summary` → HTTP 500 | `apps/api` — analytics module | Web-seller dashboard у Азима: DevTools показывает `Failed to load resource: 500` от `/seller/summary`. Контракт фронта: `{ views, topProduct: {productId,views}\|null, conversionRate }`. Посмотреть Railway-api логи за 2026-04-17 ~16:00 UTC, stacktrace покажет причину (скорее всего Prisma — `groupBy`/`count` на пустом сторе, или рассинк после недавних schema-миграций). Полный лог: `analiz/logs.md` → API-SUMMARY-500-001. |
| **API-ORDER-ADDR-001** | 🟡 Баг данных | Заказ `#741201` без `deliveryAddress` | `apps/api` (orders storage/DTO) | `OrderListItem.deliveryAddress: DeliveryAddress` в `packages/types/src/api/orders.ts:28` — required. Но заказ #741201 (production, Азим видит в `/orders`) приходит с `deliveryAddress === undefined`. Варианты: (а) старый тестовый заказ, созданный до фикса BUG-001 (checkout DTO мисматч) — null в БД; (б) какой-то код path теряет поле. Действия: посмотреть запись заказа в prod БД (`SELECT id, delivery_address FROM "Order" WHERE id = '741201';` или полный id), понять, null ли это в хранилище или фильтруется на выходе. Если старый мусорный заказ — можно просто удалить. Если реальный баг — чинить. Фронтовый guard (коммит `abb0c41`) уже на месте — UI не падает, показывает `—, —`. |
| **API-ORDER-PREVIEW-001** | 🟢 UX | Превью товара в списке заказов (фото+название) | `apps/api` + `packages/types` | Сейчас `OrderListItem` (`packages/types/src/api/orders.ts:22`) отдаёт только мету: id/status/totalAmount/deliveryAddress/createdAt, **без items**. В UI списка web-seller/TMA продавец видит только адрес и сумму — «что заказали» непонятно без клика в детали. Предложение: добавить поле `preview?: { title: string; imageUrl: string \| null; itemCount: number }` — title+image первого OrderItem и кол-во позиций. Опция 2: вернуть весь `items: OrderItem[]` в OrderListItem, но тогда OrderItem сам должен получить `imageUrl` (сейчас у него только title). После контракта — Азим отрендерит в OrderRow. Не блокер, приоритет после TMA-EDIT-001 / API-SUMMARY-500-001. **Заметка (17.04.2026 22:00):** Полат сообщил Азиму что уже добавил «опциональные поля» в `packages/types`, но в `origin/main` этого пока нет — либо локальная ветка, либо оговорка. Проверить после следующего пуша Полата. |
| **API-UPLOAD-ENV-001** | 🟡 Инфраструктура | Настроить env vars для Telegram file storage в Railway API | Railway Variables сервиса `savdo-api` | Savdo использует Telegram-канал `-1003760300539` как file storage (бот `@savdo_builderBOT` шлёт туда документы). Полат сообщил 17.04.2026 что для работы фото-upload нужны 2 переменные в Railway API: `TELEGRAM_BOT_TOKEN=8682354018:AAE7U_p2bcQLyPra3K9cjPYXx-cSw1MwaXE` и `TELEGRAM_STORAGE_CHANNEL_ID=-1003760300539`. Плюс убедиться что бот — **администратор** канала (иначе API не сможет туда отправлять). Азим не имеет доступа к Railway API Variables — это делает Полат. Web-seller сам ничего не хранит, все upload-flow через API. |
