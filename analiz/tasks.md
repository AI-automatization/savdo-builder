...
- [~] **`TMA-DESIGN-FG-TOKENS-001`** (P0, T2) — **прогресс 13.05.2026**: ВСЕ buyer pages (237) + ВСЕ seller pages в моей зоне (119) = **356 точек** мигрированы на `--tg-text-*` / `--tg-surface*` / `--tg-border*`. Light theme работает автоматически. **Осталось:** ~115 точек в TMA components (BottomNav, Sidebar, BottomSheet, CategoryModal, ImageCropper, ProductCard, SellerAnalyticsCard, StoreDirectionsPicker, ProductReviews, LoadingScreen, FullscreenButton, App.tsx). AddProduct/EditProduct/seller/SettingsPage отложены — занятые параллельной сессией. **Урок:** параллельная сессия (web-buyer/web-seller checkout) откатывает unstaged → коммитить сразу после каждого edit.
