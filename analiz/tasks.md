...
- **🆕 30.05.2026 — web-часть закрыта (Азим):** `<MaxsavdoMark>` на ветках web-buyer (`4e5d5b0`)
  ...
  Визуально сверено Playwright'ом. Деталь — `done.md`. **Не запушено.** Приоритет P0→P1: на сайте
  больше не «чужой» лого. **Остаток:** (1) точная векторизация JPG (Vectorizer.ai/Recraft) для 1:1
  + замена `d`-атрибутов; (2) favicon/app-icon/OG из вектора; (3) admin/email/TG-бот (Полат).
