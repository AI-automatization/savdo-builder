...
| ID | Важность | Кратко |
|----|----------|--------|
| `INFRA-RAILWAY-WATCH-PATTERNS-001` | 🔴 | **Web-seller и web-buyer не деплоятся в проде с 26.04 ~21:00** — все push'и сессии 36 НЕ применились. В Railway dashboard у `savdo-builder-by` Watch Patterns = `apps/tma/**`, Dockerfile Path = `apps/tma/Dockerfile` (UI перебивает наш `apps/web-buyer/railway.toml`). Поэтому каждый deploy помечается «Skipped — No changes to watched files». У `savdo-builder-sl` deploy ~17 ч назад со статусом **Failed** — нужны Build Logs (может TS-ошибка). **Фикс:** Settings → Build → Dockerfile Path = `apps/web-{buyer,seller}/Dockerfile`, Watch Patterns = `apps/web-{buyer,seller}/**, packages/types/**, packages/ui/**`. Аналогично коммиту `0fe2a92` для admin. См. `analiz/logs.md#INFRA-RAILWAY-WATCH-PATTERNS-001`. |
