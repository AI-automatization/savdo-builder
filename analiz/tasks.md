...
| ID | Важность | Кратко |
|----|----------|--------|
| `INFRA-RAILWAY-WATCH-PATTERNS-001` | 🔴 | ✅ **Готово** (Полат пофиксил Railway dashboard 28-29.04, Азим подтвердил «работает»). Можно перенести в done. |
...
| `API-CATEGORY-FILTERS-CASE-001` | 🔴 | `GET /storefront/categories/:slug/filters` шлёт `fieldType` как Prisma enum **UPPERCASE** (`"SELECT"`, `"NUMBER"`, `"TEXT"`, `"BOOLEAN"`). Локальный тип на фронте `apps/web-buyer/src/lib/api/storefront.api.ts` объявлен `'select' \| 'number' \| 'text' \| 'boolean'` — рендерер `CategoryAttributeFilters.tsx:185,228,257` сравнивает с lowercase. Все три ветки **никогда не были true** в проде → все фильтры рендерились через fallback (text input), select dropdown'ы и boolean-toggle'ы не показывались. **Фронт-фикс уже сделан 29.04** — в `getCategoryFilters` нормализую `fieldType.toLowerCase()`. Идеальный backend-фикс: добавить `@Transform(value => value.toLowerCase())` в DTO или экспортить enum в lowercase. Альтернатива — обновить тип на фронте к uppercase, но тогда нужен mapping в трёх ifах. |
| `API-NOTIFICATIONS-INBOX-CONTRACT-001` | 🔴 | `GET /notifications/inbox` возвращает `{notifications, total, unreadCount, page, limit}` (см. `apps/api/src/modules/notifications/use-cases/get-inbox.use-case.ts:36`). Локальный тип на фронте в обоих апах объявлен `{data: NotificationItem[], meta: {page, limit, total}}` — `data.data` всегда **undefined**. Это и есть корень открытой задачи `/notifications диагностика` — список нотификаций показывался пустым на seller, хотя бэк возвращал записи. **Фронт-фикс уже сделан 29.04** — `getInbox()` в обоих `apps/web-{buyer,seller}/src/lib/api/notifications.api.ts` теперь читает `data.notifications`. Желательно вынести `InboxResponse` в `packages/types/src/api/notifications.ts` (новый файл) — сейчас тип задублирован в обоих апах + не помечает `unreadCount` (полезное поле для badge). |
| `API-SELLER-PRODUCT-MEDIA-URLS-001` | 🔴 | `GET /seller/products` и `GET /seller/products/:id` (`products.controller.ts:79-83, 122-123`) спред raw Prisma product без `mediaUrls: string[]`. Repo (`products.repository.ts:73-86`) делает include `images: { include: { media: true } }`, но controller не маппит на `mediaUrls`. Тип `ProductListItem` требует `mediaUrls: string[]`. **Эффект:** seller edit page (`apps/web-seller/.../products/[id]/edit/page.tsx:298`) и список товаров читают `product?.mediaUrls?.[0]` → пустой thumbnail на собственных товарах продавца. Storefront (`/storefront/products`) маппит правильно, поэтому buyer side ОК. **Фикс:** добавить тот же `mediaUrls = images.map(img => resolveImageUrl(img.media))` в seller-controller (как делает `storefront.controller.ts`). |
| `API-SELLER-STORE-LOGO-URL-001` | 🟡 | `GET /seller/store` (`stores.repository.ts:8-13` — `findBySellerId/findBySlug/findById`) возвращает Prisma raw без resolve в `logoUrl`/`coverUrl`. Только `logoMediaId`/`coverMediaId`. Тип `Store` в `packages/types/src/api/stores.ts:5-22` объявляет `logoUrl: string \| null` обязательным. **Эффект:** в `/settings:485,497` и `/profile:189-190` `store.logoUrl` всегда `null` → продавец загружает лого через media-upload, но в превью кадра остаётся пусто (тихая UX-деградация). **Фикс:** в репозитории include `logoMedia` + `coverMedia` и resolve через тот же `resolveMediaUrl()` helper что в `cart.mapper.ts:3`. |
| `API-STORE-CONTRACT-001` | 🟡 | `GET /storefront/stores/:slug` (`products.controller.ts`) возвращает Prisma raw `Store` с полями `{logoMediaId, coverMediaId, ...}`. Тип `StorefrontStore` в `packages/types/src/api/stores.ts:36` ожидает `logoUrl, coverUrl, categories: Pick<StoreCategory, 'id' \| 'name' \| 'sortOrder'>[]`. Поля `logoUrl/coverUrl` **отсутствуют** — фронт читает через `?? null`, но картинка магазина не показывается. **Влияние:** `apps/web-buyer/.../[slug]/page.tsx`, `RecentStores.tsx`. **Фикс:** mapper в storefront-controller resolve media → URLs (тот же helper что нужен для `API-SELLER-STORE-LOGO-URL-001`). |
| `API-CHAT-MESSAGE-CONTRACT-001` | 🟡 | `POST /chat/threads/:id/messages` (`send-message.use-case.ts:25,83`) возвращает Prisma raw `ChatMessage` с полем `body` вместо `text`, без `senderRole`/`isDeleted`. `PATCH .../messages/:mid` (`chat.controller.ts:269-276`) шлёт `{id, threadId, text, senderRole, editedAt, createdAt}` — ближе, но без `isDeleted`. Тип `ChatMessage` в `packages/types/src/api/chat.ts:5-13` ожидает консистентный shape с `text/editedAt/isDeleted`. **Эффект на сегодня минимальный** — web-seller `useSendMessage` в `useMutation.onSuccess` сразу делает `invalidateQueries`, поэтому raw shape не рендерится напрямую. Но `useEditMessage` мерджит `updated.text + updated.editedAt` напрямую в кэш — если мерж получит `{body}` вместо `{text}`, отредактированное сообщение покажется пустым после клика «Сохранить» до следующего refetch. **Фикс:** ввести `mapChatMessage(m, thread)` в бэке, использовать в send + edit + list (сейчас `get-thread-messages.use-case.ts:14-22` уже маппит — копировать оттуда). |
| `API-PRODUCT-CONTRACT-002` | 🟡 | `Product.basePrice` в типе `packages/types/src/api/products.ts:53` объявлен `number`, бэк сериализует Prisma `Decimal` как **строку** (`"basePrice":"250000"`). Также `oldPrice`, `salePrice`, `priceOverride` (вариантов). **Эффект:** `ProductCard.tsx:115` и cart/checkout уже знают про это и оборачивают в `Number(...)`/`toNum()`. Но `apps/web-buyer/.../[slug]/products/[id]/page.tsx:92` делает `selectedVariantObj?.priceOverride ?? product?.basePrice ?? 0` — если оба строки, то `??`-fallback вернёт первую truthy строку, а если потом её сложить с числом → конкатенация. Latent bug. **Фикс:** type-уровень — обновить тип на `string` и через адаптер парсить в number в одном месте. ИЛИ бэк serialize Decimal как number (но потеря точности). |
| `API-SELLER-ORDERS-LIST-MAPPER-001` | 🟡 | `get-seller-orders.use-case.ts:44-69` спредит raw Prisma `Order` (subtotalAmount, discountAmount, cancelReason, placedAt, customerComment, addressLine1/2 — те же поля что чинились в `API-SELLER-ORDER-DETAIL-CONTRACT-001`). Возвращает `deliveryFeeAmount` (Prisma) вместо `deliveryFee` (тип). `totalAmount` снова Decimal-as-string. Сейчас не падает потому что `fmt(unknown)` в `apps/web-seller/.../orders/page.tsx` уже захардненена. **Фикс:** ввести `mapOrderListItem` mirror на `mapOrderDetail`. |
