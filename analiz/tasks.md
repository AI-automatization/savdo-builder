...
## 🔴 [LANDING-STALE-BOT-USERNAME-001] apps/landing указывает на СТАРЫЙ Telegram-бот в 3 местах
- **Домен:** apps/landing (Азим) · **Кто взял:** не назначено
- **Контекст:** owner заметил живьём на сайте старое имя бота (24.07.2026). Проверено grep +
  read — подтверждено. В одном и том же приложении сосуществуют два бота:
  - ✅ Правильно (`https://t.me/maxsavdo_bot`): `Hero.tsx:16-17`, `Header.tsx:20`, `Pricing.tsx:27`.
  - ❌ Старое имя (`https://t.me/savdo_builderBOT` + канал `https://t.me/savdobuilder`):
    `apps/landing/src/lib/jsonld.ts:5-6` (JSON-LD `Organization.sameAs`), `apps/landing/src/
    components/Footer.tsx:20-21` (реальная кнопка «Написать» в футере КАЖДОЙ страницы сайта),
    `apps/landing/public/llms.txt:10` (`Telegram bot: https://t.me/savdo_builderBOT` — прямым
    текстом отдаётся AI-ассистентам как факт).
- **Откуда взялось:** внесено коммитами `e0a97d5` (JSON-LD) и `5c0a041` (llms.txt) — то есть не
  унаследованный legacy-код, а моя же недавняя SEO-работа (12-16.07), скопировавшая имя бота из
  черновика/шаблона без сверки с реальным `maxsavdo_bot`.
- **Почему критично:** (1) юзер-фейсинг — клик по футеру ведёт на мёртвый/чужой бот; (2) отдаёт
  неверную информацию в JSON-LD (сигнал для Google/AI, кто мы) и в llms.txt (буквально врёт
  AI-ассистентам, каким ботом нас находить) — прямо противоречит `SEO-GEO-AEO-RESEARCH-002`, где
  llms.txt был похвален как GEO-задел без построчной сверки содержимого.
- **Что сделать:** заменить `TELEGRAM_BOT_URL`/`BOT_URL` в `jsonld.ts:5`, `Footer.tsx:20` и строку
  в `llms.txt:10` на `https://t.me/maxsavdo_bot`; канал `savdobuilder` в `jsonld.ts:6`/`Footer.tsx:21`
  сверить — если отдельного публичного канала (не бота) не существует, убрать эту ссылку вовсе,
  а не оставлять ещё один несуществующий адрес.
...
## 🔴 [WEB-SELLER-ONBOARDING-AUDIT-002] Аудит веб-онбординга продавца (23.07.2026) — 6 находок
- **Домен:** apps/web-seller (Азим) · **Кто взял:** не назначено
- **Контекст:** owner попросил проверить, можно ли регистрироваться сразу через сайт (не только
  через бота) — проверил код `(auth)/login/page.tsx` + `(onboarding)/onboarding/page.tsx` +
  `use-seller.ts` + backend `verify-otp.use-case.ts`/`create-store.use-case.ts`.
  **Хорошая новость:** прямая регистрация через сайт уже реализована и работает правильно —
  4-шаговый визард (магазин → контакты → товар → отправка на проверку), новый юзер всегда
  создаётся BUYER (`verify-otp.use-case.ts:47-51`), апгрейд в SELLER через `applySeller()` только
  при заполнении формы. Старый аудит `WEB-010`/`WEB-011` (см. `apps/web-seller/CLAUDE.md`) —
  уже пофикшен, роль после логина проверяется, редиректы корректные. Ниже — 6 новых находок.

### 🔴 1. Ошибка создания магазина глотается молча — пустой `catch {}`
- **Файл:** `(onboarding)/onboarding/page.tsx:533` (`handleStep2`) — `catch { setError('Не удалось
  создать магазин. Попробуйте ещё раз.') }`, без параметра ошибки.
- **Почему это баг:** backend реально отдаёт конкретный код `STORE_SLUG_TAKEN` (409,
  `create-store.use-case.ts:53`, "This slug is already taken"), но фронт всегда показывает один
  и тот же generic текст — продавец, выбравший занятый адрес магазина, не понимает, что именно
  не так, и жмёт "Попробовать ещё раз" с теми же данными → та же ошибка по кругу.
- **Что сделать:** `catch (err)` → достать `err.response?.data?.message` (axios,
  `apps/web-seller/src/lib/api/client.ts` не оборачивает ошибку, это сырой NestJS response) и
  показывать его, если есть, иначе — generic фолбэк.

### 🟠 2. Конфликт slug обнаруживается только на шаге 2, без live-проверки на шаге 1
- **Файл:** `Step1` (строки 131-203) — поле slug генерируется из названия, но нигде не проверяется
  на доступность до сабмита шага 2 (`createStore.mutateAsync`).
- **Проблема:** продавец заполняет шаг 1 → шаг 2 (контакты) → и только при сабмите узнаёт, что
  slug из шага 1 занят — приходится возвращаться назад. Debounce-проверка `GET /storefront/
  stores/:slug` (эндпоинт уже существует, используется на buyer-стороне) прямо на шаге 1 закрыла
  бы это заранее.
- **Решение Азима:** делать ли live-check вообще (лишний запрос на каждый keystroke) или
  достаточно фикса #1 (внятная ошибка) — вопрос приоритета, не блокер.

### 🟠 3. Товар на шаге 3 ("Первый товар") создаётся без фото
- **Файл:** `Step3` (строки 300-390) — только `title` + `basePrice`, никакого `ImageUploader`.
- **Контраст:** `(dashboard)/products/create/page.tsx:10,147` использует
  `<ImageUploader purpose="product_image" />` — то есть возможность есть, просто не подключена
  в онбординге.
- **Почему это противоречие:** подпись шага прямо гласит «Добавьте хотя бы один товар, чтобы
  магазин выглядел живым» — но товар без фото в `ProductCard` (TMA/web) рендерится с
  neutral/empty-state фоном, то есть выглядит как раз НЕ живым, а пустым/недоделанным.
- **Что сделать:** добавить `ImageUploader` в Step3 (не обязательным, но заметным полем) —
  переиспользовать компонент из dashboard/products/create.

### 🟡 4. Город — свободный текст без валидации/списка
- **Файл:** `Step2`, поле `city` (строки 262-274) — обычный `<input>`, `required` + `minLength:2`,
  никакой нормализации/списка.
- **Риск:** разнобой в написании («Ташкент» / «ташкент» / «Tashkent» / «г. Ташкент») ломает
  будущую фильтрацию/группировку магазинов по городу (если она появится в поиске/каталоге).
- **Решение Азима:** нужен ли dropdown/автокомплит по фиксированному списку городов УЗ сейчас,
  или это preMVP-полировка на потом — это продуктовое решение, не баг с однозначным фиксом.

### 🟡 5. DRAFT/PENDING_REVIEW-магазин почти незаметен в дашборде
- **Файлы:** Step4 (`onboarding/page.tsx:394-468`, кнопка «Сделаю позже, перейти в дашборд» без
  пояснения последствий) + `(dashboard)/dashboard/page.tsx:96-99` (статус магазина — мелкая серая
  подпись `text-xs`/`textDim` рядом с названием, никакого баннера/CTA).
  Оба шага (Step3 "товар" и Step4 "отправить на проверку") можно пропустить — продавец попадает
  в полностью рабочий на вид дашборд, магазин при этом невидим покупателям (DRAFT/не
  PENDING_REVIEW), и единственная подсказка об этом — едва заметная надпись.
- **Решение Азима:** нужен явный баннер-нагоняй («Ваш магазин пока не виден покупателям —
  отправьте на проверку» + кнопка) для DRAFT-статуса на дашборде — вопрос UX-приоритета,
  не туда ли важнее оставить дашборд «чистым».

### 🟢 6. Нет загрузки логотипа/аватара магазина в онбординге
- **Контекст:** ни на одном из 4 шагов нет поля под лого — TMA/web показывают заглушку
  (первая буква названия, `store.name.charAt(0)`, см. `StorePage.tsx` в apps/tma) до тех пор,
  пока продавец сам не найдёт это в настройках дашборда (если такая настройка вообще есть —
  не проверял `(dashboard)/settings`).
- **Решение Азима:** добавлять ли шаг/поле лого в онбординг сейчас (усложняет визард) или
  оставить как post-onboarding действие в настройках — приоритет самый низкий из всех находок.

### 🟡 7. (доп. 24.07, построчное чтение текста экранов) Прогресс-бар без подписей на мобильном
- **Файл:** `onboarding/page.tsx:108-113` (`ProgressBar`) — подписи шагов («Магазин»/«Контакты»/
  «Товар»/«Готово») имеют класс `hidden sm:block`, на мобильном видно только кружки-цифры/галочки
  без текста.
- **Почему это баг:** `apps/web-seller/CLAUDE.md` прямо указывает аудиторию как «часто с телефона»
  и требует mobile-first — но именно на мобильном пользователь не понимает, что означает каждый
  шаг визарда, только видит абстрактные цифры 1-2-3-4.
- **Решение Азима:** показать подписи компактно (например под прогресс-баром отдельной строкой)
  или хотя бы текущий шаг текстом рядом с прогрессом на мобильном.

### 🟢 8. (доп. 24.07) Нет кнопки «Назад» на Step4 (финальный экран)
- **Файл:** `onboarding/page.tsx:394-468` (`Step4`) — только «Отправить на проверку» и «Сделаю
  позже», нет `onBack`.
- **Проблема:** если продавец на финальном экране замечает опечатку в названии магазина
  (`storeName` показан прямо в тексте), вернуться и поправить нельзя — только после, через
  настройки дашборда.
- **Решение Азима:** низкий приоритет, но тривиально добавить `← Назад` к Step3, как у Step2/Step3.

### 🟢 9. (доп. 24.07) Чеклист на Step4 хардкоднут, не отражает реальное состояние
- **Файл:** `onboarding/page.tsx:424-428` — `[{label: "Аккаунт создан", done: true}, ...]`, все три
  пункта статично `done: true`, не читают реальный стейт (например, есть ли товар — Step3 можно
  было пропустить).
- **Почему не критично сейчас:** на момент показа Step4 все 3 пункта фактически всегда true (шаг2
  создаёт и магазин, и контакты). Но реализация — не проверка, а декорация; если позже добавится
  условный путь, где это перестанет быть правдой, никто не заметит, что чеклист начал врать.
- **Решение Азима:** низкий приоритет — можно оставить как есть, но не расширять чеклист новыми
  пунктами без реальной проверки состояния.

- **Addendum 24.07:** проверено `(dashboard)/settings/page.tsx` — загрузка лого/обложки магазина
  РЕАЛЬНО существует там (`ImageUploader purpose="store_logo"`/`"store_banner"`, строки 293-313),
  просто действительно только post-onboarding, как и предполагалось в решении по находке 6.

## 🟡 [WEB-SELLER-SCREENS-AUDIT-003] Построчный текстовый аудит остальных экранов web-seller (24.07.2026)
- **Домен:** apps/web-seller (Азим) · **Кто взял:** не назначено
- **Метод:** по прямому требованию owner — прочитан ЦЕЛИКОМ (не grep, не офсетами) каждый экран:
  `page.tsx` (root redirect), `(auth)/login`, `(dashboard)/layout.tsx` (сайдбар/хедер), `products`,
  `products/create`, `products/[id]/edit`, `orders`, `orders/[id]`, `analytics`, `chat`,
  `notifications`, `settings` (все 5 секций), `subscription`, `profile`, `store/categories`.
  Экраны без находок (чисто, не выдумываю проблемы): root redirect, login, dashboard layout/сайдбар,
  store/categories (образцовый паттерн — реальные API-ошибки через `apiError.message`, не generic).

### 🔴 1. Profile — показывает СЫРОЙ enum вместо перевода для 2 статусов магазина
- **Файл:** `profile/page.tsx:17-24` (`STATUS_LABEL`) — ключи `DRAFT/SUBMITTED/APPROVED/REJECTED/
  SUSPENDED/PUBLISHED`. Реальный enum (`packages/db/prisma/schema.prisma:31-38`, `StoreStatus`):
  `DRAFT/PENDING_REVIEW/APPROVED/REJECTED/SUSPENDED/ARCHIVED`. `SUBMITTED` и `PUBLISHED` в коде —
  несуществующие значения, `PENDING_REVIEW` и `ARCHIVED` вообще не замаплены.
- **Эффект:** для магазина в статусе "на проверке" или "в архиве" продавец на своей же странице
  профиля видит буквально `PENDING_REVIEW` / `ARCHIVED` английским текстом (fallback
  `{ text: store.status, color: colors.textDim }`) вместо "На проверке"/"Архивирован" — при этом
  `(dashboard)/dashboard/page.tsx:34-41` (`STORE_STATUS_LABELS`) те же статусы переводит правильно.
  Явное рассогласование двух копий одной и той же мапы статусов в одном приложении.
- **Что сделать:** заменить локальный `STATUS_LABEL` в `profile/page.tsx` на те же значения, что в
  `dashboard/page.tsx` (или, лучше, вынести в один общий файл, раз уже разошлись один раз).

### 🟠 2. Тарифы обещают функции, которых, похоже, нет в коде
- **Файл:** `subscription/page.tsx:23,30` (`TIERS`) — Free-тариф обещает «поддомен», Pro-тариф
  обещает «свой домен, без бейджа».
- **Проверено:** grep по `apps/api/src` на custom-domain/subdomain-логику находит только
  `main.ts:86-88` — это CORS-комментарий про НАШИ СОБСТВЕННЫЕ поддомены компании
  (`shop./seller./api.maxsavdo.uz`), не фича на продавца. `web-buyer` вообще не делает
  subdomain-based роутинг, только path `/{slug}` (подтверждено в `DEPLOY-DOMAIN-MAXSAVDO-001`).
  Кода под «отдельный поддомен продавца» или «привязать свой домен продавца» не нашёл.
- **Почему это важно:** Studio-тариф честно помечает недостроенное как `«мульти-стор (roadmap)»`
  прямо в тексте — а «поддомен»/«свой домен» на Free/Pro НЕ помечены как roadmap, то есть либо
  фича где-то реализована не под этим именем (тогда нужны файл:строка от Азима), либо это платное
  обещание фичи, которой нет. **Нужно решение Азима:** либо указать реализацию, либо снять
  формулировку/пометить как roadmap так же честно, как это сделано для мульти-стора.

### 🟡 3. Систематически: реальная ошибка от API не показывается, только generic-фраза
- **Где:** `products/create/page.tsx:302` ("Не удалось создать товар"), `products/[id]/edit/
  page.tsx:392` ("Не удалось сохранить изменения"), `orders/page.tsx → handleAction` (нет
  ErrorBanner вообще при неудаче — кнопка просто возвращается в обычное состояние без объяснения),
  `orders/[id]/page.tsx → handleForward/handleCancel` (то же самое, `finally` без `catch`-показа).
  Та же категория, что раньше нашли в онбординге (`handleStep2`, `WEB-SELLER-ONBOARDING-AUDIT-002`
  находка 1).
- **Контрпример в этом же приложении, чтобы было видно — так можно и нужно:** `chat/page.tsx:167-172`
  (`errorText()` helper) реально читает `err.response?.data?.message` и показывает конкретную
  причину при удалении/редактировании сообщений и чатов — рабочий образец для остальных экранов.
- **Что сделать:** унифицировать через паттерн `chat/page.tsx` — показывать `err.response?.data?.
  message` где он есть, generic-фразу оставлять только как fallback, не как единственный текст.

### 🟡 4. Аналитика — сырое английское перечисление статусов показано продавцу
- **Файл:** `analytics/page.tsx:282` — KPI-карточка "В работе" имеет `sub="Confirmed + Processing +
  Shipped"` — буквально английские имена enum-значений статуса заказа, единственное место на всей
  странице не на русском (все остальные подписи — "Доставленные заказы" и т.п.).
- **Что сделать:** заменить на "Подтверждён + В обработке + В пути" (те же переводы, что уже есть в
  `STATUS_LABELS` на `orders/page.tsx:12-19`).

### 🟡 5. Уведомления — счётчик "непрочитанных" обнуляется до того, как продавец что-то прочитал
- **Файл:** `notifications/page.tsx:101-105` — `useEffect(() => { readAll.mutate(); }, [])`
  срабатывает fire-and-forget сразу при монтировании страницы.
- **Эффект:** колокольчик в хедере (`(dashboard)/layout.tsx` badge `unreadCount`) обнуляется и
  вкладка «Непрочитанные» становится пустой в момент открытия страницы — до того, как продавец
  реально что-либо прочитал (просто зашёл посмотреть). Функционально работает, но название
  "Непрочитанные" вводит в заблуждение — там никогда не будет что показать через секунду после
  захода.
- **Решение Азима:** низкий/средний приоритет по ощущениям — либо помечать прочитанным по реальному
  просмотру/клику на каждое уведомление отдельно, либо явно переименовать вкладку.

### 🟢 6. Товар, скрытый админом — тупиковое сообщение без способа связаться
- **Файл:** `products/[id]/edit/page.tsx:241` — "Этот товар скрыт администратором. Обратитесь в
  поддержку для разъяснений." — текст без единой ссылки/кнопки, хотя почти везде в приложении
  (`login/page.tsx`, `subscription/page.tsx` и др.) поддержка = кликабельная ссылка на
  `@maxsavdo_bot`. Здесь просто слова "обратитесь в поддержку" без адреса.
- **Смежное:** `products/page.tsx:26-31` (`STATUS_FILTERS`) не имеет вкладки-фильтра для
  `HIDDEN_BY_ADMIN`, хотя `STATUS_LABELS`/`STATUS_COLORS` там же его знают — товар, скрытый
  админом, виден только под "Все", без объяснения причины прямо в списке.
- **Решение Азима:** добавить ссылку/кнопку на `@maxsavdo_bot` рядом с текстом на edit-странице.

### 🟢 7. «Номер аккаунта» — так подписан номер телефона покупателя
- **Файл:** `orders/[id]/page.tsx:337` — заголовок `<p>Номер аккаунта</p>` над `tel:`-ссылкой на
  `order.buyer.phone`. Формулировка звучит как ID/логин, а не как телефон для звонка.
- **Решение Азима:** низкий приоритет — переименовать в "Телефон покупателя".
