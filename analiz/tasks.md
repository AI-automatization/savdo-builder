...
- [ ] **`WEB-SELLER-STORE-CATEGORIES-CRUD-001`** 🟢 P3 (Азим) — UI страница в `/store/categories` (или похожее) для CRUD store-категорий. Сейчас seller может выбрать существующую через select в product create/edit, но не управлять списком (нет страницы). Бэкенд готов (`/seller/categories` POST/PATCH/DELETE есть в seller.api.ts:98-114).
