...
| `API-CATEGORY-SEED-CLEANUP-001` | 🟡 | **Авто нам не нужно.** В seed'е `global-categories-seed.service.ts` в числе 7 корневых категорий есть «Автомобили» (+ её дочерние вроде «Авто/мото» и связанные 6 фильтров: Марка, Модель, Год, Пробег, Двигатель, КПП, Состояние, Цвет). Платформа не для продажи авто — убрать категорию из seed (с каскадом: удалить CategoryFilter по slug + саму GlobalCategory). До этого web-seller скрывает её на фронте через `isHiddenCategory(slug in {cars, auto, automobiles} \|\| name /авто/i)`. После правки seed фронт-фильтр можно убрать. |
