...
## [2026-08-03] [LANDING-SEO-CONTENT-NOT-DEPLOYED-001] Весь SEO/GEO-контент лежит в ветке, в проде — 404
- **Статус:** 🔴 Не баг кода, а недоставка — **домен `apps/landing` (Юсуф)** + merge/деплой
- **Приоритет:** 🔴 высокий — работа сделана и не приносит ничего, пока не смержена
- **Что случилось (живая проверка 03.08.2026, `fetch_page.py` из claude-seo, не догадка):**
  ```
  maxsavdo.uz/faq              → 404
  maxsavdo.uz/qollanma         → 404
  maxsavdo.uz/ru/rukovodstva   → 404
  maxsavdo.uz/sitemap.xml      → 2 URL (/ и /ru), lastmod 2026-07-26
  ```
  Ветка `seo/landing-aeo-geo-2026-07-30` содержит `lib/guides.ts` (6 гайдов = 3 темы × uz/ru,
  23 FAQ-вопроса, 8 таблиц, 2 HowTo), `app/sitemap.ts` на ~12 URL, отдельные `/faq` и `/ru/faq`.
- **Уточнение того же дня (первая формулировка была неполной):** дело не в одной несмерженной
  ветке. Ветка **7 впереди и 10 позади** `origin/main` (merge-base `21e347f3`), а `main` тоже
  не задеплоен — его страницы из `2cc84cc6` (`/about`, `/blog`, `/cases`, `/contacts`) живьём
  тоже отдают 404. В проде сборка старше обеих веток. Открывать PR из ветки «как есть» нельзя:
  диff выглядит как удаление `de9fc86d` (partner-эндпоинты RAOS, Полат), 7 landing-страниц и
  brand-palette миграций в tma/web-buyer.
- **Почему `/faq` не «выпал из индекса»:** его никогда и не индексировали. На живой главной
  внутренние ссылки только `/` и `/ru`, в sitemap `/faq` нет, сам URL — 404. Обнаружения не
  было, поэтому в GSC он не появится даже как ошибка.
- **Как это выглядит снаружи:** сайт выглядит как одностраничник на 2 URL без единого
  informational-входа. Все запросы вида «telegramda do'kon ochish» / «сколько стоит магазин
  в телеграм» проходят мимо, хотя страницы под них написаны и лежат в репозитории.
- **Что сделано:** ничего не правил — зона `apps/landing` и решение о мерже не мои.
  Занесено в `tasks.md → LANDING-SEO-CONTENT-NOT-DEPLOYED-001` с проверкой готовности
  (`/qollanma` = 200, sitemap = 12 URL).
- **Повторяющийся класс:** третий случай расхождения landing-ветки с продом —
  ср. `LANDING-BRANCH-STALE-002` (26.07) и `LANDING-BRANCH-DRIFT-001` (08.07). Возможно,
  стоит завести отдельную проверку «ветка landing == то, что отдаёт домен».

## [2026-07-30] [BUYER-SITEMAP-TEST-STORE-001] shop.maxsavdo.uz отдаёт в sitemap тестовый магазин «ТЕСТ - удалить»
- **Статус:** 🔴 Баг, не исправлен — **домен `apps/web-buyer` (Полат)**, Юсуф не трогает (зоны CLAUDE.md)
- **Приоритет:** 🔴 высокий — GSC подключен на днях, первый краул увидит именно это
- **Что случилось (проверено живьём 30.07.2026, `Invoke-WebRequest`, не догадка):**
  `https://shop.maxsavdo.uz/sitemap.xml` отдаёт 10 URL, из них 3 — тестовый магазин и его товары:
  - `/test-udalit-ms1gi4um` — `priority 0.8`, `changefreq daily`
  - `/test-udalit-ms1gi4um/products/fef31250-3be8-4183-bb7c-36e72e609aeb`
  - `/test-udalit-ms1gi4um/products/1ce32a0c-945c-44a3-9104-3951b4e161e8`
  Страницы живые, отдают **200** (не 404, не noindex). У товара
  `<title>fdgh — ТЕСТ - удалить</title>`, `<meta name="description" content="test">`.
  `shop.maxsavdo.uz/robots.txt` даёт `Allow: /` (закрыты только `/checkout`, `/cart`, `/orders`,
  `/wishlist`, `/chats`, `/api/`) — то есть блокировать это сейчас нечем.
- **Почему так:** динамический `sitemap.ts` (закрыт в `SEO-AUDIT-001` P0 п.2, ветка `web-buyer`,
  `b215b59b`) мапит **весь** фид `GET /storefront/sitemap` без фильтра по состоянию магазина.
  Фид отдаёт всё, что опубликовано, — про «тестовый/черновой» он ничего не знает. Реальных
  магазина в выдаче всего два (`raos` + тестовый), т.е. **половина индексируемой поверхности
  субдомена — мусор** с thin-content (`description: "test"`).
- **Чем это плохо именно сейчас:** это не косметика. Thin/low-quality страницы в первом крауле
  дают сигнал качества на весь субдомен, а `changefreq daily` + `priority 0.8` прямо просят
  Google ходить туда чаще, чем на нормальные страницы.
- **Варианты фикса (решение за Полатом, здесь не выбираем):**
  1. Фильтр по состоянию магазина в `sitemap.ts` / в самом фиде `GET /storefront/sitemap`
     (нужен признак test/draft — сейчас его в контракте `StorefrontSitemapFeed` нет).
  2. `robots: { index: false }` в `generateMetadata` для таких магазинов.
  3. Оперативно, независимо от кода: удалить тестовый магазин из прод-БД
     (по образцу `PROD-DB-CLEANUP-001`, done.md 16.07) + GSC → Removals.
- **Связанное:** `SEO-AUDIT-001` (sitemap стал динамическим), `PROD-DB-CLEANUP-001` (чистка базы
  от smoke-данных — тот же класс: тестовые сущности доживают до прода).
...
## [2026-07-30] [SEO-TEST-STORE-INDEXED-001] Тестовый магазин уходил в индекс Google
- **Статус:** 🟡 Код-защита есть (ветка `seo/web-buyer-index-hygiene-2026-07-30`),
  данные всё ещё нужно почистить в админке
- **Что случилось:** после подключения Search Console проверил живой `shop.maxsavdo.uz` и
  нашёл, что магазин `test-udalit-ms1gi4um` («ТЕСТ - удалить») — approved и публичный в
  проде. Он лежал в `sitemap.xml` с `priority 0.8` / `changefreq daily`, был слинкован с
  главной из блока «Топ магазины» (тот самый server-rendered блок из SEO-AUDIT-001 п.3) и
  отдавал self-referencing canonical. Два его товара — тоже в sitemap. Живых магазинов
  всего два (`raos` + этот), то есть половина индексируемого — магазин с названием
  «ТЕСТ - удалить».
- **Корневая причина:** не код, а данные — тестовый магазин прошёл approval в проде и
  никто его не удалил. Все SEO-механизмы (динамический sitemap, SSR-ссылки на главной,
  canonical) отработали ровно как задумано, просто на мусорных данных. Урок: как только
  витрина стала по-настоящему индексируемой, любая тестовая запись в проде становится
  публичной страницей — гигиена тестовых данных теперь часть SEO, а не только QA.
- **Что сделано:** `apps/web-buyer/src/lib/seo/index-exclusions.ts` — явный список слагов
  (не regex `/test|demo/`, иначе выпадет реальный продавец со словом «test» в названии),
  применён в трёх местах сразу, потому что каждого по отдельности недостаточно: sitemap
  (магазины + их товары), `robots` в метаданных страницы магазина и страницы товара, плюс
  фильтр featured-блока на главной.
- **Что осталось:** удалить/снять с публикации сам магазин в админке, после чего слаг
  можно убрать из списка. Проверить заодно, нет ли других тестовых магазинов.

## [2026-07-30] [WEB-BUYER-TSCONFIG-TYPES-001] tsc падает на несуществующих type-библиотеках
- **Статус:** 🟡 Предупреждение, не чинил (ветка `web-buyer`, не мой заход)
- **Что случилось:** `npx tsc --noEmit` в `apps/web-buyer` даёт 2 ошибки TS2688 —
  «Cannot find type definition file for 'vitest/globals' / '@testing-library/jest-dom'».
  Проверил на чистом `origin/web-buyer` (через `git stash`) — те же 2 ошибки до любых
  моих правок, то есть это не регрессия.
- **Корневая причина:** `apps/web-buyer/tsconfig.json:16` объявляет
  `"types": ["vitest/globals", "@testing-library/jest-dom"]`, но ни `vitest`, ни
  `@testing-library/jest-dom` не входят в `devDependencies` web-buyer. Похоже, конфиг
  скопирован из приложения с тестами, а сами тесты в web-buyer так и не завели.
