...
- **Статус:** 🔴 Найдено, НЕ исправлено (только аудит + ручная перепроверка). Продолжение вчерашнего
  ...
  web-buyer главная/discovery+`/wishlist`+`ChatComposerModal`.
- **Метод:** 4 параллельных `feature-dev:code-reviewer` агента на реальных деплой-ветках через
  disposable git worktree (`origin/web-buyer`@`02082a7`, `origin/web-seller`@`d4bb40c`, свежий
  `pnpm install` в обоих). Каждая находка перепроверена мной вручную прямым чтением файла/строки.
  12 находок всего, 10 CONFIRMED, 2 PLAUSIBLE (структурно верны, не до конца прослежены все caller'ы).

### web-seller `/notifications` (не разбирался вчера вообще)
1. 🔴 **CONFIRMED** `hooks/use-seller-socket.ts:88-90` — фронт вообще не подписан на `notification:new`,
   хотя бэкенд его уже шлёт специально взамен поллинга (см. `apps/api/.../chat.gateway.ts:50-54` и
   `in-app-notification.processor.ts:36-46`, комментарии там прямо говорят "frontend больше не
   poll'ит"). `useUnreadCount` (`use-notifications.ts:23-32`) живёт только на `refetchInterval: 30_000`.
   Если продавец сидит на `/notifications` когда прилетает push — новая запись НЕ появится в списке
   вообще, пока не уйдёт со страницы и не вернётся (или refocus окна).
2. 🟡 **CONFIRMED** `hooks/use-notifications.ts:34-43` — `useReadAll()` без `onError` (в отличие от
   соседней `useUpdateNotifPreferences`). Сбой mark-all-read полностью тихий — продавец думает что
   отметил всё прочитанным, а бэкенд ошибку вернул.
- **Побочная находка (не web-домен, для Полата):** `apps/api/.../notification.service.ts:134`
  `notifyInApp()` нигде не вызывается ни из одного use-case (заказы/чат/чекаут) — даже после фикса
  фронта пуш-канал будет пустым, пока бэкенд не начнёт реально enqueue'ить уведомления.

### web-seller `/settings` + онбординг-визард (вчера поверхностно)
3. 🔴 **CONFIRMED** `(dashboard)/settings/page.tsx:275-277,408` — смена лого/обложки магазина
   (`ImageUploader` грузит файл сразу и вызывает `onChange(mediaId)`) НЕ переключает `isDirty`
   (это чисто RHF-состояние, `logoMediaId`/`coverMediaId` — отдельный `useState` вне формы). Кнопка
   "Сохранить" остаётся задизейбленной → загруженная картинка никогда не привязывается к магазину,
   после перезагрузки — старое лого, без единого сигнала продавцу.
4. 🟡 **CONFIRMED** `(dashboard)/settings/page.tsx:151,222-232` — форма доставки нигде не
   деструктурирует `errors` из `formState` и не передаёт их в `Field` (в отличие от соседних секций
   того же файла). Невалидная сумма доставки (`"15.5"` проходит HTML `type=number`, но не
   `/^\d+$/`) блокирует сабмит молча — ни ошибки, ни сохранения, ни объяснения.
5. 🟡 **CONFIRMED** `(onboarding)/onboarding/page.tsx:495` — `<Step1 onNext={handleStep1} />` не
   получает `defaultValues`/`step1Data` обратно. Шаг размонтируется при переходе на Step2, и "Назад"
   рендерит Step1 с пустыми полями — название магазина/slug теряются, вводить заново.
6. 🟡 **CONFIRMED** `(onboarding)/onboarding/page.tsx:291,419-433` — кнопка Step2 дизейблится только
   `createStore.isPending || updateProfile.isPending`, но НЕ на время `await applySeller()` (отдельный
   ручной await до этих мутаций). Быстрый двойной клик BUYER'ом в этом окне может выстрелить
   `POST /seller/apply` дважды параллельно → риск для INV-S01 (один продавец = один магазин), той же
   природы что уже чинили для `Promise.all`-кейса (см. комментарий в коде на строках 440-442), но не
   для этого пути.

### web-buyer `ChatComposerModal` (вчера не разбирался детально)
7. 🔴 **CONFIRMED** `components/chat/ChatComposerModal.tsx:34-37,79` — Escape и клик по фону закрывают
   модалку БЕЗ проверки `create.isPending` (в отличие от кнопки Cancel, которая правильно
   задизейблена на `isPending`). На медленной сети покупатель может закрыть модалку во время отправки:
   при успехе фоновый `await` всё равно доходит до `onClose()` (no-op) + `router.push("/chats")`
   (строки 67-68) — юзера форсированно перекидывает на `/chats`, хотя он думал что отменил; при ошибке
   `setError(...)` (строка 71) вызывается на уже размонтированном компоненте — ошибка теряется молча,
   покупатель не узнает что тред/сообщение не создались.
8. 🟡 **PLAUSIBLE** `components/chat/ChatComposerModal.tsx:31-59`, deps `[onClose]` — оба caller'а
   (`product/[id]/page.tsx:837`, `orders/[id]/page.tsx:524`) передают инлайн-стрелку `() =>
   setChatOpen(false)`, новая ссылка на каждый ре-рендер родителя → focus-trap эффект пересоздаётся и
   на cleanup форсит `.focus()` на элемент "до модалки" (скрытую под оверлеем кнопку), что может
   выдёргивать фокус из textarea при фоновом refetch родительской страницы (window focus). Не
   прослеживал до конца все условия рефетча — правдоподобно, не 100% подтверждено.

### web-buyer главная/discovery + `/wishlist` (вчера не разбирался детально)
9. 🟡 **CONFIRMED** `hooks/use-wishlist.ts:88-92` — `onError` только откатывает optimistic-кэш, нигде
   (ни в `ProductCard.tsx`, ни в `wishlist/page.tsx`) не показывается юзеру ошибка. Сердечко тихо
   прыгает обратно без объяснения — тот же класс бага что вчера в степпере корзины.
10. 🟢 **CONFIRMED** `hooks/use-wishlist.ts:41-43,66-83` — optimistic insert при добавлении в вишлист
    реализован, но НИ ОДИН caller (`ProductCard.tsx:45`, `products/[id]/page.tsx:109`,
    `wishlist/page.tsx:114`) не передаёт `productPreview` → ветка мёртвая, сердечко всегда ждёт
    resolve мутации + инвалидацию, видимая задержка на каждый тап (removal — ок, там путь другой).
11. 🟢 **CONFIRMED** (минор) `components/layout/BottomNavBar.tsx:14,43` — `NavActive` объявляет
    `'wishlist'`, `wishlist/page.tsx:34` рендерит `active="wishlist"`, но в массиве `NAV` пункта
    wishlist нет — активная вкладка никогда не подсвечивается на `/wishlist`. Чисто косметика.
- **Не web-домен (для Полата, к сведению):** `apps/api/.../get-wishlist.use-case.ts:35-39` —
  `isAvailable` считается по `status/isVisible/store.status/isPublic`, стока там нет вообще (и
  репозиторий его не селектит) → распроданный товар в вишлисте продолжает выглядеть доступным, OOS-
  оверлей на карточке невозможен без бэкенд-изменения контракта.
- **Проверено, багов нет:** двойной тап по сердечку wishlist (guard есть, `cancelQueries` тоже),
  дедуп по id на главной/wishlist (списки не пагинированные/не аккумулируются), hydration на
  главной/wishlist (localStorage-чтения нет), `lib/auth/context.tsx` (вчерашний фикс держится).
- **Definition of done для темы:** решение Азима — фиксить сейчас (какие из 10) или только
  зафиксировать как тикеты на потом.

## [2026-07-03] [WEB-AUDIT-BUYER-SELLER-002] Полный аудит web-buyer + web-seller + landing — 14 багов + 1 инфра-блокер
- **Статус:** ✅ Все 14 багов исправлены и запушены **напрямую в деплой-ветки** тем же днём:
  `web-buyer` коммит `02082a7`, `web-seller` коммит `d4bb40c`. `tsc --noEmit` чист в обоих (свежий
  `pnpm install` в disposable git worktree — не в основном чекауте, см. урок ниже).
  Инфра-блокер (лендинг недоступен, Railway не оплачен) — НЕ фронт-фикс, остаётся открытым.
- **Побочная находка при фиксе:** `WEB-BUYER-ENUM-AS-VALUE-001` (заведён этой же сессией чуть раньше)
  оказался ложной тревогой — стухший pnpm-symlink в основном чекауте, не реальный баг на `web-buyer`.
  Retracted, см. `tasks.md`. **Урок:** проверять чужую ветку — только через `git worktree` + `pnpm
  install` в нём, не переключением веток в общем чекауте с общим `node_modules`.
- **Метод:** 6 параллельных `feature-dev:code-reviewer` агентов по подсистемам (read-only, без
  build/tsc), каждый — на **реальных деплой-ветках** через disposable git worktree (`origin/web-buyer`
  @ `9227e66`, `origin/web-seller` @ `64c5971`), НЕ на `feat/seller-landing` (сильно разошлась, см.
  `OTP-BOT-MISMATCH-001` выше). Каждая находка перепроверена вручную (прямое чтение файла/строки) —
  10 из 14 подтверждены CONFIRMED, 4 остались PLAUSIBLE (согласуются с кодом, не перечитаны построчно).
- **web-buyer (7 находок):**
  1. 🔴 `checkout/page.tsx:287` — поле города инициализировано литералом `"Ташкент"`, а не пустым
     плейсхолдером; `canSubmit` пропускает это как «заполнено» → покупатель вне Ташкента может отправить
     заказ с неверным городом доставки незаметно для себя.
  2. 🔴 `lib/auth/context.tsx:98` — mount-эффект логаутит юзера на ЛЮБОЙ ошибке `getMe()` (не только 401)
     — сетевой сбой/5xx на плохом соединении выкидывает из сессии валидного пользователя.
  3. 🟡 `[slug]/products/[id]/page.tsx:199` — `handleAddToCart` без обработки ошибок → INV-C01
     (корзина = один магазин) конфликты и сбои стока падают молча, без единого UI-фидбека.
  4. 🟡 `cart/page.tsx:46` — `QtyStepper` +/- не блокируется на время mutation, `adjustQty` считает
     от последнего server-confirmed `item.quantity` → быстрые тапы гонятся и теряют инкременты.
  5. 🟡 `cart/page.tsx:84` — ошибки `update`/`remove` мутаций нигде не читаются → нет фидбека на
     отказ (например, инкремент сверх остатка).
  6. 🟡 `(shop)/orders/page.tsx:126` — аккумулятор `accOrders` не дедуплицирует по id; любой фоновый
     рефетч текущей страницы (WS-инвалидация статуса заказа, refocus вкладки) дублирует уже показанные
     заказы на экране (page > 1).
  7. 🟡 `[slug]/products/[id]/page.tsx:112` — `activeImage` не сбрасывается при переходе на другой
     товар (сброс есть только для варианта/selection) → возможен `images[activeImage] === undefined`
     на `next/image` src при переходе на товар с меньшим числом фото.
  8. 🟡 `lib/auth/context.tsx:32` — `user` инициализируется из localStorage прямо в `useState`-лениво
     во время hydration-рендера → структурный SSR/CSR mismatch на всех auth-gated страницах для
     вернувшегося залогиненного покупателя (вспышка логин-экрана + hydration-ошибки в консоли).
- **web-seller (6 находок + landing):**
  1. 🔴 `(dashboard)/layout.tsx:385` — SUSPENDED read-only оверлей — `position:absolute;inset:0` внутри
     скроллящегося `main` → скроллится вместе с контентом, реально закрывает только «above the fold».
     Backend хард-гейтит только создание товара — это ЕДИНСТВЕННАЯ линия защиты для SUSPENDED-продавца,
     и она обходится простым скроллом любой страницы длиннее экрана. **Прямой revenue-риск для
     BILLING-MACHINE-001.**
  2. 🔴 `products/[id]/edit/page.tsx:619` — кнопка «В архив» показывается и для DRAFT-товара, хотя
     backend разрешает только `ACTIVE→ARCHIVED` (подтверждено в `change-product-status.use-case.ts`)
     — `DRAFT→ARCHIVED` получает 422, но `handleStatusChange` без try/catch и `updateStatus.isError`
     нигде не читается → продавец не понимает что произошло.
  3. 🟡 `(dashboard)/layout.tsx:336` — статус `CHURNED` (хуже чем SUSPENDED по стейт-машине) не имеет
     НИ баннера, НИ оверлея — дашборд выглядит полностью нормальным.
  4. 🟡 `multi-image-uploader.tsx:80` — батч-аплоад фото прерывается на первой ошибке ДО вызова
     `onChange` → уже успешно загруженные фото (существуют на сервере) теряются из UI, продавец
     перезаливает всё заново вслепую.
  5. 🟡 `product-variants-section.tsx:360` — `handleUpdate` (редактирование варианта) без try/catch,
     в отличие от соседнего `handleAdd` (уже чинили этот же паттерн) — тихий сбой на дубль-SKU/невалидной
     цене.
  6. 🟡 `hooks/use-seller-socket.ts:74` — обработчик `chat:new_message` только показывает toast, не
     инвалидирует query тредов → бейдж непрочитанных в сайдбаре не обновляется в реальном времени,
     пока продавец не на странице чата.
  - **🟢 Инфра, не код-баг (подтверждено живым в проде):** `web-seller` `/` = `redirect('/dashboard')`
    (коммит `7b0d7b3`, 19.06.2026, осознанное решение — «лендинг переехал на сервис `landing`»).
    Реальный лендинг живёт на `deploy/seller-landing` → Railway `landing` — задеплоить не может, т.к.
    **Railway-подписка просрочена** (см. `done.md` 30.06.2026 `LANDING-DEPLOY-MERGE`). Итог: **публичный
    лендинг НЕ доступен ни по одному живому URL** минимум с 30.06 (5+ дней на момент аудита). Не фронт-
    баг — упирается в оплату Railway (см. `DEMO-BUTTON-RAILWAY-001`, тот же корень, но там речь только
    про одну кнопку — здесь весь маркетинговый лендинг).
- **Не баги (агенты проверили и опровергли):** OTP `purpose='login'` дефолт в `OtpGate` (web-buyer) —
  раньше создавал SELLER, но backend уже правлен (`verify-otp.use-case.ts` всегда создаёт BUYER
  независимо от purpose) — **документы `WEB-010` в `apps/web-seller/CLAUDE.md` и `done.md` устарели**,
  не описывают текущее поведение бэка; reveal-on-scroll на лендинге; chat reconnect/rejoin; blob-URL
  cleanup; order-status transition gating (buyer+seller) — всё корректно, регрессий нет.
- **Что сделано:** только аудит + верификация чтением, код не менялся (кроме OTP-BOT-MISMATCH-001
  ранее в этой же сессии). Дальнейшие фиксы — по решению после разбора приоритетов с Азимом.
