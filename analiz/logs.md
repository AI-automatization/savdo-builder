...
- **Что проверено (живьём, Playwright + curl, не догадка):** все 28 роутов (uz+ru, включая
  динамические `/qollanma/*`, `/blog/*`, `/ru/rukovodstva/*`) → 200 OK. Консоль браузера — 0 ошибок
  на `/`, `/ru`, `/qollanma/telegramda-dokon-ochish`. Статические ассеты (лого, фото, шрифты) —
  без 404. Нет горизонтального оверфлоу на 390px. Критерии из `LANDING-MULTIPAGE-EXPANSION-001` и
  честные тарифы (`LANDING-PRICING-FALSE-CLAIMS-001`) — подтверждены живьём, на месте.
- **Найдено:**
  1. 🔴 `LANDING-MOBILE-NAV-MISSING-001` — на 390px в шапке нет ни nav (`hidden md:flex`), ни CTA
     (`hidden sm:inline-flex`), ни гамбургер-кнопки замены. Регресс относительно старой ветки
     `landing`, где мобильное меню было.
  2. 🟡 `LANDING-LANG-TOGGLE-DOWNGRADE-001` — переключатель языка откатился с круглой pill-кнопки
     (оба языка видны, transition) на плоскую прямоугольную ссылку с одним языком.
  3. 🟡 `LANDING-FAQ-DEAD-CONTENT-001` — 5 FAQ-ответов из `main` (`faqCategories` в `i18n.ts`)
     объявлены, но нигде не рендерятся — тот самый открытый вопрос из описания PR #7, до сих пор
     не решён.
- **Не баг, отдельно на заметку:** 8 пунктов nav в шапке — на 1280px+ помещается нормально
  (проверено скриншотом), сам Юсуф пометил в PR как "may want a design pass" — не критично.
...
## [2026-07-28] [LANDING-PROD-MISMATCH-001] Живой maxsavdo.uz не совпадает с веткой `landing` — причина не найдена
- **Статус:** 🟡 Открыто — исследование прервано, ждёт Railway-доступа или ответа Юсуфа
- **Что случилось:** после LANDING-SELLER-DEDUP-001 (см. done.md 28.07) закоммитил и запушил
  `landing` (`0e88833d..b896b070`, подтверждено `git fetch`+`ls-remote` — коммит реально на GitHub).
  Азим открыл живой `https://maxsavdo.uz/` — контент не совпадает ни с моей веткой, ни с версией
  ДО моих правок:
  - Есть мой текст дословно: `WhyUs` («Nega maxsavdo, hozirgisidan koʻra» + вся сравнительная
    таблица), `ProblemSection` («Direct yoki storiesda sotyapsizmi?»), `FinalCta`
    («Doʻkoningizni 5 daqiqada ishga tushiring»).
  - НЕТ моего: `SocialProof`-секции нет вообще; `Header`/`Footer` — старые (nav без «Doʻkonlar
    katalogi», footer 2 колонки без «Hujjatlar»/«Mahsulot»), не мои новые 4-колоночные.
  - Есть контент, которого я **вообще не писал**: в Hero два доп. блока «KATALOG»/«BUYURTMALAR»
    с описаниями, живые тосты «Savatga qoʻshildi» / «Yangi buyurtma #1049» поверх мокапа телефона.
  - `FeaturedStores` отдаёт реальные прод-данные (магазин «ТЕСТ - удалить», Tashkent) — значит это
    точно живой прод-рендер, не какой-то dev-кэш браузера.
- **Проверено и исключено:**
  - `curl -sI https://maxsavdo.uz/` → `x-nextjs-cache: HIT`, `Server: railway-hikari`,
    `Cache-Control: s-maxage=3600, stale-while-revalidate=31532400` — то есть это реальный
    Railway/Next.js ISR-кэш, не сторонний CDN и не браузерный кэш Азима.
  - `git fetch origin landing` → `origin/landing` HEAD = `b896b070`, ровно мой коммит. Новых
    коммитов от кого-либо ПОСЛЕ меня на ветке `landing` нет.
- **Гипотеза (не подтверждена):** Railway-сервис `landing` либо задеплоен не из ветки `landing`
  этого репо (другой root directory / другая ветка / вручную через Railway CLI без git), либо
  Юсуф (владеет доменом `apps/landing` с 27.07) катал что-то отдельно от git-истории, которую я
  читал. Смешение «моего текста» и «неизвестного контента» на одной странице этим объясняется
  лучше всего, но не проверено — нужен Railway-дашборд (branch/root dir/deploy log сервиса
  `landing`) или прямой ответ Юсуфа, что именно он деплоил и как.
- **Важно:** дальше ничего не трогать/не чинить вслепую — есть риск затереть незакоммиченную
  (или закоммиченную не в этот репо) работу Юсуфа. Ждать либо Railway-логина от Азима, либо
  ответа Юсуфа.

**Апдейт 04.08.2026:** причина найдена — прод деплоится с `main` (не `landing`), но деплой от
31.07 висел в `INITIALIZING` из-за истёкшего Railway-биллинга. После оплаты Redeploy пересобрал
старый закреплённый коммит `79eb74fd`, а не актуальный `main`; нужен принудительный fresh-деплой.
...
- **⚠️ Обновление 01.08.2026 (Азим смотрел Billing напрямую):** всё ещё ОТКРЫТО, спустя
  больше месяца. Причина отказа карты — «insufficient funds». Live-проверка curl'ом
  01.08 (maxsavdo.uz/api/shop/seller) — все сервисы пока Online, отключения ещё не было.
  Invoice-история (usage-based subscription, помесячно): 02 → $20.00, 03 → $27.44,
  04 → $59.85, 05 → $59.44, 06 → $34.04, **07 (24.06-24.07) → $87.78, неоплачен** — заметный
  скачок, больше чем в 2 раза к предыдущему циклу. Прямого доступа к разбивке Usage (per-service/
  per-resource) в этой сессии не было (нет Railway CLI/логина) — не подтверждено, что именно
  подорожало. Гипотезы (grounded, не проверены дашбордом), по активности в `done.md` за 24.06-24.07:
  1. `BACKUP-001` (18.07) — добавлена ежедневная автоматическая `pg_dump` → R2 задача, новый
     постоянный recurring compute/network job, которого не было в предыдущих циклах.
  2. `PARTNER-API-RAOS-001` (14.07) — RAOS-интеграция пошла в прод, скачивает фото по https и
     раздаёт их дальше через Telegram storage — трафик через Railway «удваивается» (см. заметку
     в `analiz/tasks.md` про trade-off с R2).
  3. Высокая деплой-активность в цикле: DNS-cutover 5 доменов (07.07, много redeploy),
     SEO-фиксы (10.07-25.07, много коммитов/деплоев), landing build-топология сломана/чинена
     несколько раз (`LANDING-DEPLOY-TOPOLOGY-001` 25.07) — Railway тоже биллит build-minutes,
     не только runtime.
  4. `PERF-API-001` (18.07) — новый composite-индекс, небольшой прирост Postgres storage/compute.
  - **Проверить (Полату/Азиму в Railway dashboard):** вкладка **Usage** (не Billing) — там
    разбивка $ по сервису и по типу ресурса (compute/network egress/build minutes/storage).
    Это единственный способ подтвердить, какая именно гипотеза верна.
  - **⚠️ КОРРЕКЦИЯ 01.08.2026 (тот же день):** гипотезы выше были сделаны в узком scope —
    только репо `savdo-builder`. Азим уточнил, что Railway-биллинг общий на весь **Workspace
    TezCode** (Billing-скрин показывал именно «pro Workspace», не отдельный проект). Проверка
    через GitHub API (`search/commits`, org `AI-automatization`, то же окно 24.06-24.07) дала
    другую картину: `Rave` — **315 коммитов**, подтверждён Railway (`.railwayignore` в корне);
    `hamshiraGo` — **268 коммитов**, подтверждён Railway (`backend/railway.toml`); для сравнения
    `savdo-builder` — только 116 коммитов за то же окно. Т.е. в масштабе всей организации
    maxsavdo, вероятно, НЕ главный источник скачка счёта — `Rave`/`hamshiraGo` разрабатывались
    в этом цикле в 2-3 раза интенсивнее и оба подтверждённо на Railway. Как и раньше — это прокси
    (частота коммитов), не сами $; окончательное подтверждение только через Railway Usage,
    сгруппированный **по Project** (не по сервису внутри одного проекта).
