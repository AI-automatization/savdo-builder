...
- **Статус:** 🟡 Открыто (рассинхрон, найден при buyer-тесте 29.06).
...
- **Находки:** (1) greeting «Привет, .!» — у buyer-header СВОЙ компонент, BUG-TMA-1 имеет
  buyer-близнеца (тот же fallback `first_name||username` нужен и тут). (2) «ФОТО НЕДОСТУПНО»
...
- **Статус:** 🟡 Открыто (UX/функциональный пробел). Репорт владельца 29.06.2026.
