...
- `WB-B01` доставка не считается (money)
- `WB-B02` нельзя удалить товар из корзины
- `WB-B04` нет error-UI на checkout (вместе с checkout-500 — слепой экран)
- `WS-B01` неверный OTP ломает логин продавца
- `WS-B02` онбординг-ловушка для новых продавцов
- `WS-B03/B04/B05` потеря фото при загрузке/edit
- `WS-B06` data-corruption вариантов на `" / "`
- `WS-B09` дубли атрибутов из-за POST-per-keystroke
- `WS-B10` create product — молчаливый partial-success
