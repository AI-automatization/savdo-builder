...
- **Что сделано:**
  ...
  2. **Динамические placeholder'ы** — словарь `TITLE_EXAMPLES_BY_SLUG` (23 slug'а) даёт релевантный пример: `phones → iPhone 15 Pro`, `shoes → Nike Air Max`, `cars → Chevrolet Cobalt 2018, 1.5L`, `furniture → Офисное кресло с сеткой`, `books → Мастер и Маргарита, Булгаков`, `laptops → MacBook Pro 14 M3` и т.д. Unknown slug → `Например: товар из категории «{name}»` (новые категории Полата работают без кода). Без выбора категории — нейтральное `Название товара`.
  3. **Placeholder описания тоже контекстный** — для одежды просит размерную сетку, для авто — год/пробег/КПП, для электроники — характеристики/гарантию, для книг — автор/жанр/год.
  4. **Переименовал старую «Категория» (storeCategoryId) → «Раздел магазина»** — чтобы не путать с глобальной. Seller-local категория остаётся опциональной.
  5. **Цена placeholder** `10000` → `10 000` (читабельнее).
