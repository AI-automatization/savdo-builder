...
## 2026-07-07 (Азим) — LANDING-DEPLOY-RESTORE-001: публичный лендинг снова живой на web-seller `/`

### ✅ [LANDING-DEPLOY-RESTORE-001] `app/page.tsx` переключён с redirect('/dashboard') обратно на LandingPage
- **Важность:** 🔴 (готовит прод под наведение домена maxsavdo.uz)
- **Дата:** 07.07.2026
- **Контекст:** оказалось, что весь премиум-лендинг (components/landing/*, lib/landing/*, i18n-словарь,
  CSS keyframes, env vars, картинки) уже был смержен в `origin/web-seller` ещё в июне — только
  `app/page.tsx` был откачен на `redirect('/dashboard')` 19.06 (решение «лендинг переехал на отдельный
  Railway-сервис landing», который так и не задеплоился из-за просроченной Railway-подписки). Компоненты
  никто не убирал — просто page.tsx их не рендерил.
- **Что сделано:** `apps/web-seller/src/app/page.tsx` — заменён redirect на `LandingPage` (as-is из
  `feat/seller-landing`, поверх уже присутствующих в web-seller компонентов — НЕ копировал файлы поверх,
  origin's версии `LandingHeader.tsx`/`analytics.ts` новее и правильнее моих, копирование их бы
  регрессировало `nav.toCabinet`-фикс).
- **Попутно найдены и исправлены 2 спящих i18n-бага** (не ловились tsc, вскрылись бы только когда
  лендинг реально стал рендериться): `SocialProof.tsx` ссылался на `'proof.betaBody'`, в словаре есть
  только `'proof.betaDesc'` → без фикса на лендинге был бы виден сырой ключ вместо текста. `why.row2` —
  проверил отдельно, ложная тревога (существует в uz.ts, просто в двойных кавычках из-за апострофа).
- **Проверка:** `tsc --noEmit` чист, `next build` чист, живой смоук-тест локального prod-билда
  (`next start` + curl) — `200 OK`, верный `<title>`, hero-текст рендерится, 0 сырых i18n-ключей в HTML.
- **Коммит:** `36142b1` на `origin/web-seller` (`3d4afb1..36142b1`), запушено напрямую.
- **Не сделано (не код, зона Полата/инфры):** сам домен maxsavdo.uz — DNS не настроен (проверено:
  `nslookup` даёт SERVFAIL, домен нигде не резолвится). Наведение DNS на этот Railway-сервис — следующий
  шаг, ждёт доступа к AHOST/Cloudflare.

## 2026-07-07 (Азим) — WEB-AUDIT-BUYER-SELLER-003: 8/9 находок раунда 3 исправлены, запушены

### ✅ [WEB-AUDIT-BUYER-SELLER-003] Раунд 3 аудита (заказы/товары/каталог/профиль) — 8 фиксов, запушены напрямую в web-buyer/web-seller
- **Важность:** 🔴 2 критичных + 🟡 5 важных (8/9; 1 минор сознательно пропущен)
- **Дата:** 07.07.2026
- **Метод:** правки в disposable worktree на актуальных деплой-коммитах (`.worktrees/audit-web-buyer-0707`
  @ `origin/web-buyer` bb6cf91, `.worktrees/audit-web-seller-0707` @ `origin/web-seller` e2dd0f9), свежий
  `pnpm install` в обоих, `tsc --noEmit` чист. Запушено напрямую: `web-buyer` (`bb6cf91..972198f`),
  `web-seller` (`e2dd0f9..3d4afb1`).
- **web-buyer (5 находок, коммит `972198f`):**
  - `orders/[id]/page.tsx` — `canCancel` сужен до `PENDING` (было `PENDING|CONFIRMED`, нарушение state
    machine — `CONFIRMED→CANCELLED` только seller); ошибка отмены (`cancelOrder.isError`) теперь рендерится
  - `hooks/use-orders.ts` — `useCancelOrder` получил `onError` (инвалидирует orders-кэш, чтобы реальный
    статус подтянулся после отклонённой отмены)
  - `hooks/use-buyer-socket.ts` — добавлены слушатели `notification:new` (инбокс не обновлялся live,
    зеркало seller-фикса от 05.07) и `chat:unread:bump` (бэкенд уже шлёт это персонально-в-user-room
    событие именно для кросс-тредового обновления списка чатов — фронт никогда его не слушал; более точный
    фикс, чем предполагал агент про «возможно осознанное упрощение»)
  - `stores/page.tsx` + `StoresFilters.tsx` — сортировка «Новые» убрана: бэкенд не отдаёт `publishedAt`
    в payload каталога, `.reverse()`-воркэраунд давал не «новые сверху», а буквально противоположный порядок
- **web-seller (3 находки, коммит `3d4afb1`):**
  - `orders/page.tsx` — `pendingId` (единый scalar на все строки) заменён на `Set<string>` — раньше клик по
    заказу B во время pending заказа A снимал disabled с B раньше времени (общий `finally` от A), позволяя
    двойной сабмит на B
  - `orders/page.tsx` + `orders/[id]/page.tsx` — ошибки смены статуса заказа теперь показываются (были
    вычислены, но нигде не рендерились — кнопка просто переставала крутиться без объяснения)
  - `store/categories/page.tsx` — guard на `isPending` добавлен в `handleCreate`/`saveEdit` (Enter-путь не
    проверял pending, кнопка проверяла — двойной Enter создавал дубль-категорию)
- **Не тронуто (сознательно, низкая уверенность + риск регрессии):** `stores/page.tsx` фильтры могут
  разойтись с URL — латентно, недостижимо ни одним текущим флоу; фикс двусторонней синхронизации рискует
  завести цикл ре-рендеров ради нереализуемого сейчас сценария. Оставлено как заметка в `logs.md`.
...
- **Важность:** 🔴 4 критичных + 🟡 10 важных
- **Дата:** 03.07.2026
- **Метод:** фиксы сделаны в disposable git worktree на каждой реальной деплой-ветке (не на
  `feat/seller-landing` — та разошлась), с чистым `pnpm install`, `tsc --noEmit` чист на обеих.
- **web-buyer** (коммит `02082a7`, 7 файлов):
  - `checkout/page.tsx` — город больше не предзаполнен реальным «Ташкент»
  - `lib/auth/context.tsx` — логаут только на реальный 401 (не на любую сетевую ошибку) +
    исправлен SSR/CSR hydration mismatch (`user` стартует с `null`, гидратируется в эффекте)
  - `[slug]/products/[id]/page.tsx` — обработка ошибок add-to-cart (было полностью тихо, включая
    INV-C01 конфликты корзины между магазинами); `activeImage`/`addError` сбрасываются при смене товара
  - `cart/page.tsx` — степпер количества блокируется на время mutation (гонка на быстрых тапах);
    ошибки update/remove теперь показываются построчно
  - `(shop)/orders/page.tsx` — аккумулятор списка заказов дедуплицирует по id (фоновый рефетч
    дублировал уже показанные заказы на page > 1)
- **web-seller** (коммит `d4bb40c`, 8 файлов):
  - `(dashboard)/layout.tsx` — SUSPENDED-оверлей больше не обходится скроллом (был `position:absolute`
    внутри скроллящегося `main`, реально закрывал только «above the fold»); статус CHURNED теперь
    гейтится наравне с SUSPENDED (баннер + read-only оверлей)
  - `(dashboard)/pricing/page.tsx` — CHURNED получил danger-бейдж и баннер с CTA (было — обычный серый)
  - `products/[id]/edit/page.tsx` — кнопки «В архив»/«В черновик» теперь показываются только для
    валидных исходных статусов (backend разрешает переходы только из ACTIVE, было можно словить
    422 из DRAFT/ARCHIVED без объяснения); добавлен error-баннер на смену статуса
  - `multi-image-uploader.tsx` — успешные загрузки в батче больше не теряются при сбое одного файла
    (flush после каждого успеха, а не одним разом в конце)
  - `product-variants-section.tsx` — `handleUpdate` (редактирование варианта) получил try/catch +
    error-баннер, как у соседнего `handleAdd`
  - `hooks/use-seller-socket.ts` — `chat:new_message` теперь инвалидирует query тредов → бейдж
    непрочитанных в сайдбаре обновляется live, не только когда открыт конкретный тред
- **Не тронуто (инфра, не фронт-код):** публичный лендинг всё ещё недоступен — ждёт оплаты Railway
  (см. `tasks.md` / `logs.md` `WEB-AUDIT-BUYER-SELLER-002`, тот же корень что `DEMO-BUTTON-RAILWAY-001`).
...
- **Важность:** 🔴 (разблокирует биллинг-машину и public launch)
- **Дата:** 14.06.2026
- **Файлы:** `docs/business/business-model-v2-2026-05-31.md` (§15), `docs/business/billing-machine-spec-v1-2026-05-31.md` (§12)
- **Что решено:**
  - **Цены:** Free (0) / Pro 149 000 / Studio 399 000 сум/мес
  - **Триал:** 30 дней для новых пользователей (бета-когорта — стандартные условия)
  - **Лимит тарифа:** заказы/мес; Free ≤ 50; 3 дня предупреждение → блокировка новых заказов
  - **Биллинг-машина:** Phase 0, блокер до public launch
  - **Multi-store:** Free=1, Pro=1, Studio=до 3 магазинов
  - **Grace-период:** 7 дней PAST_DUE → SUSPENDED
  - **Данные после ухода:** 90 дней хранение → удаление
  - **TG-уведомления продавцу:** в v1
  - **Beta-когорта:** `tier=PRO, currentPeriodEnd=2026-09-01, lastPaymentNote='beta_grandfather'`
  - **Контент:** узбекский первичный, русский вторичный
  - **Инфра-косты:** $11/мес Railway
- **Следующий шаг Полату:** стартовать `BILLING-MACHINE-001`, первым — `SubscriptionDto` в `packages/types`
- **Следующий шаг Азиму:** после DTO — фронт-баннеры `[BILLING-FRONT]`
...
- **Важность:** 🔴 (главный экран выглядел пустым)
- **Дата:** 07.06.2026 · **Файлы:** `components/landing/Hero.tsx`
- **Что сделано:** Hero был единственной секцией с классами `.reveal` без вызова `useReveal()` →
  весь левый текст застрял на `opacity:0`. Привязан observer к каждому блоку (стаггер сохранён).
  Пустые серые плитки в мок-телефоне заменены на витрину в стиле TMA StorePage.

### ✅ [LANDING-PHONE-TMA-001] Мок-телефон = дизайн TMA + реальные фото + локализация
- **Важность:** 🟡 · **Дата:** 07.06.2026
- **Файлы:** `Hero.tsx`, `public/landing/p-*.jpg` (4 royalty-free, Unsplash License)
- **Что сделано:** интерьер телефона повторяет TMA: шапка магазина (аватар+название+✓+рейтинг),
  поиск, чипы категорий, сетка карточек с фото/рейтингом/скидкой, нижняя навигация (Корзина•2).
  Полная локализация RU/UZ через i18n.

### ✅ [LANDING-QLAY-GAPS-001] Отстройка от qlay — showcase, соцпруф, ниши, живое демо
- **Важность:** 🟡 · **Дата:** 07.06.2026
- **Файлы:** `Showcase.tsx` (переписан), `SocialProof.tsx` (новый), `LandingPage.tsx`,
  `lib/landing/demo-store.ts`, `lib/i18n/{ru,uz}.ts`, `.env.example`,
  `public/landing/s-{fashion,beauty,jewelry}.jpg`
- **Контекст:** аудит qlay.uz (`docs/business/competitor-deep-dive-qlay-2026-06-04.md`).
- **Что сделано (код-разрывы закрыты):** (1) секция «Витрины» — 3 курируемых премиум-примера с
  реальными обложками + рейтинг/ниша + CTA на демо (доказательство «выглядит дорого»); (2) полоска
  соцпруфа «8 из 10 клиентов в Telegram» + founding-бета (честно, без фейк-счётчиков); (3) строка
  ниш в Hero; (4) включено живое веб-демо (`azim-mnx4na25`) — отстройка «демо без бота».
- **Осталось (НЕ код, Азим/Полат):** онлайн-оплата Click/Payme, цена/вход (§15), AI-фичи.
- **Проверка:** `pnpm --filter web-seller build` зелёный, tsc 0, Playwright-скриншоты (RU+UZ).
...
- **Важность:** 🔴
- **Дата:** 07.06.2026
- **Файлы:** `docs/business/payments-legal-tax-2026-06-07.md` (запушен веткой `docs/payments-legal-tax-2026-06-07`)
- **Что сделано:** решено — maxsavdo чистый SaaS, деньги сделки мимо нас (Вариант A), **комиссии с GMV нет**
  (отменён Phase 2 hybrid), доход только подписка. Способы оплаты заказа под реалии УЗ. Разобран юр/налог.
  путь: ИП/ООО/самозанятый/IT Park/Payme-Click/налоги 2026 + чек-лист Азиму. Через `superpowers:brainstorming`.
...
- **Файлы:**
  ...
  - `apps/web-seller/src/__tests__/smoke/buyer-url.test.ts` — 7 тестов на
    ...
    (pure helpers, `NEXT_PUBLIC_BUYER_URL` env + maxsavdo.uz fallback).
...
- **Файлы:**
  ...
  - `apps/web-buyer/src/__tests__/smoke/HelpContent.test.tsx` — 3 теста: 8 h2,
    intro с `support@maxsavdo.uz`, h1 title.
...
- **Файлы:**
  ...
  - `apps/web-buyer/src/app/help/page.tsx` (new) — server wrapper с metadata
    (title «Помощь и частые вопросы — maxsavdo», description).
...
- **WB-B05** — checkout: протухший токен запирал на форме (`confirm` молча
  401). Слушатель `maxsavdo:auth:expired` → возврат на OTP-шаг. Убран мёртвый
  `isAuthed`.
...
- `packages/ui/README.md` — migration plan для Азима: добавить `"@maxsavdo/ui": "workspace:*"` в web-buyer + web-seller package.json → `pnpm install` → удалить дубли → заменить импорты на `'use client'` обёртку.
...
- **Что НЕ сделано:**
  - **packages/ui подключение** — `@maxsavdo/ui` сейчас не в dependencies web-* (только tokens/colors.ts существует, но не используется). Подключение требует `pnpm install` локально (Азим не разрешает). Пока — две идентичные копии в web-buyer и web-seller, sync вручную, с явным SYNC-комментарием в обоих файлах.
...
- **Что сделано:**
  - **/become-seller** — friendly explainer для BUYER: «У вас ещё нет магазина», 3 CTA (Открыть магазин / Перейти к покупкам → maxsavdo.uz / Выйти из аккаунта). Self-guard: SELLER со store → /dashboard; не залогинен → /login
...
### ✅ [WEB-SELLER-HARDCODED-DOMAIN-001] Все maxsavdo.uz → env helper 🟠
- **Дата:** 11.05.2026
...
- **Что сделано:** добавлены `buyerHostDisplay()` и `buyerProductUrl(slug, productId)` в helper. Все 5 live мест (dashboard/products/onboarding × 2/layout placeholder) теперь используют `process.env.NEXT_PUBLIC_BUYER_URL ?? 'https://maxsavdo.uz'` через helper. В dev (`localhost:3001`) — реалистичные ссылки, не dead production. Закрывает аудит 06.05.
...
- **Что сделано:** `<html lang="en">` → `<html lang="ru">` (баг с launch). Создан `sitemap.ts` (homepage + 4 legal pages, weekly/yearly), `robots.ts` (allow /, disallow checkout/cart/orders/wishlist/chats/api), `manifest.ts` (maxsavdo PWA). JSON-LD `Organization` в root layout (sitewide). JSON-LD `Product` со schema.org/Product (name/image/sku/brand/offers/UZS) на product layout — берётся из существующего SSR fetch, no extra request.
...
- **Дата:** 08.05.2026
- **Файлы:**
  - `apps/web-seller/src/lib/buyer-url.ts` (новый) — `buyerOrigin()`/`buyerStoreUrl(slug)`/`buyerStoreDisplay(slug)` читают `process.env.NEXT_PUBLIC_BUYER_URL` с fallback на `https://maxsavdo.uz`.
...
- **Что сделано:** до этого 3 места хардкодили `https://maxsavdo.uz/${slug}` — на dev/staging юзеры видели/копировали мёртвую прод-ссылку. Теперь dev = `localhost:3001/slug`, prod = `maxsavdo.uz/slug`.
...
- **Дата:** 08.05.2026
- **Что сделано:** grep по `web-buyer/src` подтвердил: длинных railway URL в UI нет. `app/layout.tsx:16` уже на `process.env.NEXT_PUBLIC_BUYER_URL || 'https://maxsavdo.uz'`. `app/(shop)/page.tsx:94,116` — короткий label `maxsavdo.uz/` как фасад на homepage (не URL). Закрываю задачу без изменений.
...
- **Палитры:**
  ...
  - **brand wordmark** «maxsavdo» — единый `#7C3AED` в light, `#A78BFA` в dark (оба контрастно читаются на своей теме)
...
- **⚠️ Action required (Polat / infra):** установить `STORAGE_PUBLIC_URL` на Railway api сервисе. Без этого даже `/media/proxy/:id` redirect (302) сломан — он сам внутри использует `getPublicUrl`. Значение: ваш R2 public URL — либо `https://pub-xxxx.r2.dev` (default Cloudflare R2 public), либо ваш CDN-домен (`cdn.maxsavdo.uz` если настроен).
...
- **Подтверждение прод-деплоя buyer (без локального запуска):**
  - DOM evaluate на https://savdo-builder-by-production.up.railway.app: header `<a>` «maxsavdo» и hero `<span>` «maxsavdo» оба с `color: rgb(124, 58, 237)` = `#7C3AED`. Brand violet token доехал до прода.
  - `<title>` корректный «maxsavdo — магазины в Telegram». 0 errors в первой партии console (до того как Playwright-сессия закрылась).
- **Что НЕ сделано:**
...
### ✅ [WEB-BRAND-WORDMARK-UNIFY-001] Единый violet `#7C3AED` для логотипа maxsavdo на обоих фронтах 🟢
- **Дата:** 30.04.2026
...
- **Что сделано:**
  ...
  - Применён `colors.brand` в 5 местах wordmark «maxsavdo»:
...
- **Что сделано:** Каждый раз вводить slug стал только первый раз — после открытия магазина он остаётся на главной в блоке «Недавние магазины» (до 8 последних). Кнопка X на карточке локально удаляет из списка. Данные только в localStorage (`maxsavdo:buyer:recent-stores`), между устройствами не синхронизируются — полноценные favorites требуют бэка (отдельной задачи).
...
- **Цепочка бага:** Клик «Выйти» → `POST /auth/logout` со старым токеном → 401 → axios interceptor пробует `/auth/refresh` → тоже 401 → `clearTokens()` + dispatchEvent `maxsavdo:auth:expired` → AuthProvider слушает → вызывает `logout()` снова → goto step 1. Console: бесконечно `logout:1 401`.
- **Что сделано:**
...
- **Что сделано:** Рядом с копированием web-ссылки добавлена вторая кнопка (голубая иконка плоскости) которая копирует `https://t.me/<BOT_USERNAME>?startapp=product_<id>`. Клик по такой ссылке в Telegram → TMA открывается сразу на странице товара. Новая переменная `NEXT_PUBLIC_TG_BOT_USERNAME` в .env.example (fallback `savdo_builderBOT`). Заодно починил хардкод `maxsavdo.uz` в existing copy — теперь использует `NEXT_PUBLIC_BUYER_URL` (уже был в env).
...
- **Что сделано:** `generateMetadata` теперь использует coverUrl (шире для Telegram) с фолбэком на logoUrl. Добавлены twitter card, og:type, og:siteName, og:url, og:locale. Title формата "{store} — maxsavdo".
