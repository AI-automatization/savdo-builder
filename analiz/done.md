...
## 2026-08-01 (web-buyer) — SEO-CRAWL-PATH-001: у реальной витрины не было ни одного пути обнаружения

### ✅ [SEO-CRAWL-PATH-001] SSR каталога /stores + фолбэк featured-блока главной

- **Важность:** 🔴 P1 — из-за этого единственный настоящий магазин платформы не индексировался вообще.
- **Дата:** 01.08.2026
- **Файлы:**
  - `apps/web-buyer/src/lib/api/storefront-server.ts` (новый `serverGetStoresCatalog`)
  - `apps/web-buyer/src/hooks/use-storefront.ts` (`useStoresCatalog(initialData?)`)
  - `apps/web-buyer/src/app/(shop)/stores/page.tsx` (client → server component)
  - `apps/web-buyer/src/app/(shop)/stores/StoresCatalogClient.tsx` (новый, вынесенное клиентское тело)
  - `apps/web-buyer/src/app/(shop)/page.tsx` (фолбэк topStores)
- **Что сделано:**
  - **Диагноз (Search Console, 01.08.2026).** Подключили GSC — в индексе 3 страницы из 12. У всех девяти непроиндексированных одна и та же причина: `URL неизвестен Google`, `Последнее сканирование: Отсутствует`. Не отказ по качеству — Google про них просто не знал. Crawl stats: `maxsavdo.uz` 46 запросов, `shop.maxsavdo.uz` — **2**.
  - **Почему.** `/stores` — единственная hub-страница на все витрины — была client-only: в первом HTML заголовок, фильтры и пустая сетка, ни одного `<a href="/{slug}">`. Единственная ссылка на shop-поддомен со всего landing вела на тестовый магазин. Слаг `raos` не встречался ни в HTML главной, ни в `/stores`, ни в `/products`. Путей обнаружения — ноль.
  - **`/stores` теперь server component.** Клиентское тело вынесено в `StoresCatalogClient`, данные приходят SSR-фетчем как `initialData` — тот же приём, что уже стоял на главной (`serverGetFeatured` → `HomeTopStores`). Фильтры/сортировка/URL-state работают как раньше.
  - **Грабли: первый заход не сработал.** После первой итерации `raos` лежал в RSC-пейлоаде, но `<a href="/raos">` в разметке не появился. Причина — `useSearchParams()` уводит компонент в client-side bailout, ближайший `Suspense fallback={null}` отдаёт пустоту, и в статическом HTML на месте сетки был `null`. Начальные фильтры перевели на проп с сервера (`searchParams` страницы), `useSearchParams` и обёртку `Suspense` убрали. Страница стала `ƒ` (dynamic) — осознанный размен: краулеру нужен HTML, а не ISR.
  - **Регрессия главной, которую поймали до деплоя.** `8ab0a6c9` фильтрует исключённые магазины из featured-блока, но на проде `/storefront/featured` отдаёт **только тестовый магазин** — после фильтра на главной осталось бы **ноль** ссылок на витрины, хотя `raos` есть в `/storefront/stores` (просто не в featured). Добавлен фолбэк: если после фильтра `topStores` пуст — добираем публичный список. Лишний фетч только в этом случае.
  - **Проверка (build + `next start` против прод-API):** `tsc --noEmit` exit 0; `/` и `/stores` отдают `href="/raos"`, `test-udalit` — 0 вхождений; canonical на месте на `/`, `/stores`, `/products`; sitemap 9 URL (`/stores` и `/products` добавлены, тестовый магазин и его 2 товара ушли — на проде было 10 URL с ними).
- **Не сделано, осознанно:**
  - **`/products` остался client-only.** `useSearchParams` там читается реактивно на каждый рендер (не только начальное состояние) + `useInfiniteQuery` — рефакторинг тяжелее. Главное: индексируемых товаров сейчас **ноль** (`raos` — 0 товаров, товары тестового исключены), SSR отрендерил бы пустую сетку и проверить фикс нечем. Делать, когда в каталоге появятся реальные товары.
  - **`lang="ru"` / hreflang не трогали.** Это задокументированное архитектурное решение (`app/layout.tsx`): пер-локальных URL нет, hreflang указывать не на что. Менять — значит вводить `/uz/...`, это продуктовое решение, не метаданные.
- **Потолок теперь не в коде:** у `raos` 0 товаров. Код доводит краулера до витрины, смотреть там пока нечего.
