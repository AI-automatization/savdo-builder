...
## 2026-08-08 (Claude) — ✅ [LANDING-SEO-CONTENT-NOT-DEPLOYED-001] Закрыто — контент реально в проде

- **Важность:** 🔴 (была) · **Дата закрытия:** 08.08.2026 · **Домен:** `apps/landing`
- **Контекст:** задача была открыта 03.08.2026 — весь informational-слой (гайды, FAQ, sitemap)
  лежал в ветке, но не доезжал до прода из-за истёкшего триала Railway (биллинг). Билинг
  разморозили 04.08 (`project_maxsavdo_railway_billing_blocked` в памяти), PR #7 смержен.
- **Проверено живьём 08.08.2026 (не на слово):**
  - `maxsavdo.uz/sitemap.xml` → **48 `<url>` записей** (было 2 на 03.08, 12 ожидалось после PR #7,
    сейчас больше — добавились `kassa`/`ombor`-гайды из PR #22).
  - `maxsavdo.uz/faq` → реально отдаёт 11 вопросов-ответов (комиссии, устройства, заказы, лимиты
    бесплатного тарифа, оплата/аутентификация), не 404 и не заглушка.
- **Итог:** блокер по доставке контента снят полностью, дальше это уже не merge/деплой-проблема.
  Что осталось по SEO/GEO — отдельные задачи в `tasks.md` (`LANDING-ENV-SEO-MEASURE-001` — аналитика/
  verification env vars, `SEO-SHOP-*` — сторона Полата, `apps/web-buyer`/`apps/api`).
