...
- **Кто взял:** Полат (весь код кроме landing + бизнес/монетизация/надзор) | Юсуф (landing — контент+код, SEO/GEO/AIO)
...
> **Реорганизация 04.08.2026 (v3):** все прошлые зоны Азима (надзор/анализ команды, академические/бизнес-вопросы,
> бизнес-модель/монетизация/ценообразование) перешли к Полату. Азим больше не имеет отдельной зоны в проекте.
> Landing (контент и код) остаётся у Юсуфа без изменений. Всё остальное — у Полата, как и было с 27.07.2026 (v2).

| Участник | Домен | Нельзя трогать |
|---------|-------|----------------|
| Полат | **Весь код кроме landing**: `apps/api`, `apps/admin`, `packages/db`, `packages/types`, `apps/mobile-buyer`, `apps/mobile-seller`, `apps/web-buyer`, `apps/web-seller` **+ все прошлые зоны Азима**: бизнес-модель, монетизация, ценообразование, надзор/анализ команды, академические/бизнес-вопросы проекта | `apps/landing`, landing-ветка |
| Юсуф | Маркетинг, контент, GTM, продажи + **весь домен `apps/landing`/landing-ветка** (контент, SEO/GEO/AIO-анализ, и код) | Весь остальной код |
| ~~Азим~~ | ~~Академические/бизнес-вопросы, надзор, бизнес-модель~~ — перешло к Полату 04.08.2026 (v3) | — |
...
**apps/landing** — только Юсуф. Полат не трогает без явного запроса Юсуфа.

**Бизнес-модель, монетизация, ценообразование, надзор/анализ команды — Полат** (с 04.08.2026, v3; было у Азима).
**Маркетинг, GTM, контент, продажи, весь landing (контент+код) — Юсуф**, без изменений.
Полат — вся инженерия кроме landing (api/admin/db/types/tma/web-buyer/web-seller + DevOps/биллинг-cron)
**и теперь весь бизнес-слой** (раньше — только consult по техвыполнимости). Мастер-план:
