...
## PATCH `https://api.maxsavdo.uz/api/v1/partner/products/:id`

Частичное обновление (все поля опциональны, шлём только изменившееся).

```json
{ "name": "iPhone 15 Pro 256GB", "price": 15900000, "description": "...", "imageUrl": "https://cdn.raos.uz/...", "isActive": true }
```

| Поле | Тип | Примечание |
|------|-----|------------|
| `name` | string ≤200 | → `title` |
| `price` | number > 0 | → `basePrice` |
| `description` | string ≤4000 | |
| `imageUrl` | string, https | Заменяет обложку (старая удаляется, новая скачивается и заливается так же, как при создании) |
| `isActive` | boolean | `true` → публикация (DRAFT/ARCHIVED→ACTIVE), `false` → снять с витрины (ACTIVE→ARCHIVED). Идемпотентно — если товар уже в целевом статусе, no-op. |

Ответ `200`: `{ "id", "title", "status", "basePrice" }`.

## PATCH `https://api.maxsavdo.uz/api/v1/partner/products/:id/stock`

```json
{ "stock": 42 }
```

Абсолютный остаток (не дельта). Поддерживается только для single-SKU товаров
(без вариантов) — партнёрские товары создаются без вариантов, поэтому это
покрывает 100% партнёрского каталога. Ответ `200`: `{ "id", "totalStock" }`.

## DELETE `https://api.maxsavdo.uz/api/v1/partner/products/:id`

Если товар `ACTIVE` — сначала автоматически архивируется (наш INV-P04 не
позволяет удалить активный товар напрямую), затем soft-delete. Партнёру не
нужно делать двухшаговый вызов. Ответ `200` (не `204`) с телом
`{ "id", "deleted": true }`.

### Ошибки (update/stock/delete)

| Код | Когда |
|-----|-------|
| `401 UNAUTHORIZED` | нет/невалидный ключ |
| `403 FORBIDDEN` | товар принадлежит другому магазину (чужой ключ) |
| `404 PRODUCT_NOT_FOUND` | товар не найден / уже удалён |
| `422 VALIDATION_ERROR` | стоковое поле у товара с вариантами через `/stock`, или невалидное фото |
| `422 PRODUCT_PRICE_INVALID` | `price` ≤ 0 |
...
5. ~~Нужна ли синхронизация обновлений~~ — сделано 31.07.2026 (`INTEG-RAOS-002`, issue #5): update/stock/delete добавлены, контракт выше.
