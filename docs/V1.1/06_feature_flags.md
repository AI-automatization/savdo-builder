...
| `SUBSCRIPTIONS_ENFORCEMENT_ENABLED` | boolean | `false` | Phase 1: soft warnings only. Когда станет `true` — активирует `PlanLimitGuard` hard blocks (отказ при превышении лимитов плана). |
| `SUBSCRIPTIONS_AUTO_TRIAL_ON_SELLER_CREATE` | boolean | `true` | Автоматически стартовать TRIAL-подписку при создании нового seller'а. |
...
SUBSCRIPTIONS_ENFORCEMENT_ENABLED=false   # Phase 1 — soft warnings only
SUBSCRIPTIONS_AUTO_TRIAL_ON_SELLER_CREATE=true
```
...
export const featuresConfig = registerAs('features', () => ({
  ...
  subscriptionsEnforcementEnabled: process.env.SUBSCRIPTIONS_ENFORCEMENT_ENABLED === 'true',
  subscriptionsAutoTrialOnSellerCreate: process.env.SUBSCRIPTIONS_AUTO_TRIAL_ON_SELLER_CREATE !== 'false',
}));
```
