...
**Дата:** 2026-05-20
**Статус:** Recommendation document (применение `templates/scoring-matrix.md`)
**Оценщики:** Полат (backend / стратегия, выставил скоры) + симуляция Азима (web / дизайн)
**Связано:** [`docs/brand/README.md`](../brand/README.md), `analiz/audits/web-buyer-vs-seller-design-2026-05-20.md`

> **Важно:** документ — **рекомендация по числам**, НЕ финальное решение.
> Финальный sign-off — Азим как владелец дизайн-зоны (web-buyer, web-seller).
> Палитра Terracotta + cream уже **зафиксирована в коде** Азимом в session 52
> (05.05.2026, 10/10 task'ов Soft Color Lifestyle закрыто, commits
> `c117723`/`654f067`/`b2884bb`/`0ba9561`) и в dark-варианте session 55
> (09.05.2026, commit `b894589`). Этот документ скорее объясняет почему
> зафиксированное решение работает, чем выбирает между альтернативами.
...
**Рекомендация по NAME: оставить SAVDO** (статус-кво подтверждён по числам, не по инерции).
Финальное решение — за Азимом, но разрыв с #2 в 1.60 балла даёт уверенность,
что переименование на T-26 не оправдано.
...
**Рекомендация по PALETTE: TERRACOTTA + CREAM (D)** — это палитра, которую Азим
зафиксировал в коде ещё в session 52 (05.05.2026, plan «Soft Color Lifestyle»)
и расширил dark-вариантом в session 55 (09.05.2026). Scoring выше — формальное
обоснование уже принятого решения, а не выбор между альтернативами на чистом
листе. Кандидаты A/B/C — гипотетические, никогда не были в коде.
...
**Рекомендация (direction, не финальное лого):** комбинировать **A Monogram «S» + D Ornament-accent** —
основной знак-монограмма S, но её внутренняя геометрия / отрицательное пространство
построено на узбекском орнаментальном паттерне (звезда / рапид). Даёт max и L1, и L4.
Финальное исполнение (vector-iterations, размеры, версии 1-color/full-color) —
за Азимом или приглашённым дизайнером, scoring здесь только определяет
**направление**, не итоговый файл.
