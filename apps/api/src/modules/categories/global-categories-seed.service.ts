...
export class GlobalCategoriesSeedService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    this.logger.log('Seeding global categories...');
    ...
    for (const cat of sorted) {
      ...
      const record = await this.prisma.globalCategory.upsert({
        where: { slug: cat.slug },
        update: { nameRu: cat.nameRu, nameUz: cat.nameUz, sortOrder: cat.sortOrder },
        create: {
          slug: cat.slug,
          nameRu: cat.nameRu,
          nameUz: cat.nameUz,
    ...
    this.logger.log(`Seeded ${sorted.length} global categories`);
  }
}
