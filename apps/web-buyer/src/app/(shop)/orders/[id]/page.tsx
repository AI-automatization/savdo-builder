...
const STATUS_META: Record<
  string,
  { label: string; eta: string; tone: StatusTone }
> = {
  [OrderStatus.PENDING]: {
    label: "Ожидает подтверждения",
    eta: "Продавец скоро рассмотрит заказ",
    tone: "warning",
  },
  [OrderStatus.CONFIRMED]: {
    label: "Подтверждён",
    eta: "Магазин готовит ваш заказ",
    tone: "brand",
  },
  [OrderStatus.PROCESSING]: {
    label: "В обработке",
    eta: "Идёт сборка заказа",
    tone: "brand",
  },
  [OrderStatus.SHIPPED]: {
    label: "В пути",
    eta: "Курьер скоро привезёт",
    tone: "brand",
  },
  [OrderStatus.DELIVERED]: {
    label: "Доставлен",
    eta: "Спасибо за покупку",
    tone: "success",
  },
  [OrderStatus.CANCELLED]: {
    label: "Отменён",
    eta: "Заказ был отменён",
    tone: "muted",
  },
};
...
function StatusHero({ order }: { order: NormalizedOrder }) {
  ...
  return (
    ...
    >
      ...
      <div className="text-[11px] opacity-85">{meta.eta}</div>
    </div>
  );
}
        ...
        return (
          <div key={step.key} className="flex items-start gap-3">
            <div
              ...
              style={{ minHeight: i === TIMELINE.length - 1 ? "auto" : 36 }}
            >
              <div
                ...
                style={{
                  background:
                    completed || current ? colors.brand : colors.divider,
              ...
              {i < TIMELINE.length - 1 && (
                <div
                  ...
                  style={{
                    background: completed ? colors.brand : colors.divider,
                  }}
                />
              )}
            </div>
...
function PageSkeleton() {
  return (
    <div className="px-4 py-5 flex flex-col gap-3">
      ...
      />
      <div
        className="h-3 w-40 rounded-full animate-pulse mt-3"
        style={{ background: colors.surfaceMuted }}
      />
    </div>
  );
...
export default function OrderDetailPage({
  ...
  return (
    <div
      ...
      style={{ background: colors.bg, color: colors.textStrong }}
    >
      {/* Header bar */}
      <div
      ...
      <div className="max-w-3xl mx-auto pb-44 md:pb-12">
        ...
        {isError && (
          <div className="text-center py-16 px-4">
            ...
            <Link
              href="/orders"
              className="text-xs mt-3 inline-block font-semibold"
        ...
        {order && (
          <>
            ...
            <div
              style={{ height: 1, background: colors.divider }}
              className="mx-4"
            />
            ...
            <div className="px-4 py-4" style={{ background: colors.surface }}>
              ...
              {order.buyerNote && (
                ...
                >
                  ...
                  <p className="text-xs" style={{ color: colors.textBody }}>
                    {order.buyerNote}
                  </p>
                </div>
              )}
