...
function SidebarContent({ pathname, pendingCount, unreadChatCount, store, userPhone, logoutPending, onLogout, onCopyLink }: SidebarProps) {
  ...
  const NAV = NAV_ITEMS.map((item) => ({ ...item, label: t(item.key) }));
    ...
    <>
      ...
      <div className="px-3 py-4" style={{ borderTop: `1px solid ${colors.divider}` }}>
        <div className="px-3 py-2.5 rounded-lg" style={{ background: colors.surfaceMuted, border: `1px solid ${colors.border}` }}>
          <p className="text-[10px] uppercase tracking-widest mb-1" style={{ color: colors.textDim }}>{t('nav.yourStore')}</p>
          <div className="flex items-center justify-between gap-2">
            <span className="text-xs truncate" style={{ color: colors.textMuted }}>
              {store ? buyerStoreDisplay(store.slug) : `${buyerHostDisplay()}/...`}
            </span>
            <button
      ...
      >
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] truncate" style={{ color: colors.textDim }}>{t('nav.personalCabinet')}</p>
          </div>
        </Link>
        <button
          onClick={onLogout}
          ...
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-sm font-semibold" style={{ color: colors.textPrimary }}>
              {NAV_ITEMS.map(item => ({ ...item, label: t(item.key) }))
                .find(n => pathname === n.href || (n.href !== "/dashboard" && pathname.startsWith(n.href)))?.label
                ?? (pathname.startsWith("/profile") ? t('nav.personalCabinet') : t('nav.dashboard'))}
            </h2>
          </div>
          <div className="flex items-center gap-3">
            {/* Theme toggle */}
