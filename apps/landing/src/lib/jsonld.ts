 ...
 * закрывает тег и превращает данные в исполняемый код (stored XSS).
 * Экранируем `<`, `>`, `&` и U+2028/U+2029 в \uXXXX — JSON-парсер браузера
 * читает их как исходные символы, HTML-парсер уже не видит тег.
 *
 * Данные landing сейчас статические, но правило должно быть единым: иначе
 * следующая динамическая вставка снова принесёт дыру (см. issue #8).
 */
