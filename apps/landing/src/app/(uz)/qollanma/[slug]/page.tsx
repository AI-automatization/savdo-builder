...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!guide) {
    // No guide, no indexable page — returning empty metadata would still emit the
    // layout defaults and invite indexing of a 404.
...
export default async function GuidePage({ params }: PageProps) {
  ...
  const ruTwin = guides.ru.find((g) => g.key === guide.key);
  ...
  return (
    <>
      ...
      <Header
        ...
        dict={dict}
        anchorBase="/"
        switchHref={ruTwin ? guidePath("ru", ruTwin.slug) : "/ru"}
      />
      <main>
